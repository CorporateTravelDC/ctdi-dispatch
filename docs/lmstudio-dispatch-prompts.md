...
>
> **Do not copy anything out of this file into a live config.** Specifically:
> LM Studio is not installed and nothing listens on `:1234`; the
> `LOCAL_AI_ENABLED` / `LOCAL_AI_BASE_URL` / `LOCAL_AI_MODEL` variables in the
> "Config suggestion" section near the bottom are read by **no code in this
> repo** (`grep -rn LOCAL_AI_ src/` → zero hits) — the real knob is
> `OLLAMA_BASE_URL` in `dispatch.env`. The Qwen2.5/Mistral recommendations are
> also moot: `build-models.sh` builds all 21 `corporatetraveldc-pi5-*` models
> `FROM phi3:mini` and hard-blocks gemma bases for brief-class models. Re-derive
> the live set with `ollama list | grep corporatetraveldc-pi5`. (Banner
> re-verified against the live box 2026-08-23: 21 models, 21 repo-root
> Modelfiles, all `phi3:mini`; `ss -ltn` shows nothing on `:1234`, and
> `which lms lmstudio` finds neither.)
>
> **One section below is more wrong than the rest — read this before the
> "CPS Recompute" section (added 2026-08-23).** That section is written as
> though CPS is the platform's most LLM-critical skill ("This skill MUST
> return valid JSON matching the exact schema below", `response_format:
> {type: "json_object"}`, "Qwen2.5 7B Q4 — Phi-3.5 Mini is marginal for
> JSON schema"). **`cps_recompute.py` makes no LLM call of any kind
> today.** Its entire import list is `argparse` / `logging` / `sys` /
> `datetime` / `common.db` / `common.sr1_log` / `common.sr2_gate` — no
> `common.llm`, no `anthropic`, no `ollama`
> (`grep -n "^import\|^from" src/poller/skills/cps_recompute.py`). CPS
> scoring is deterministic Python against the Part 135.609 thresholds; the
> schema below is produced by code, not generated by a model, so no model
> choice, JSON mode, or prompt tuning can affect it. Anyone regionalizing
> or re-platforming inference should skip that section entirely rather
> than sizing a model for a workload that does not exist. (The skill is
> still on the poller's `SKILL_SCHEDULE` at a 3600 s interval and still
> logs through SR-1/SR-2 — deterministic, not retired.)
