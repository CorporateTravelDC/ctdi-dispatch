...
- **`thermal-ingest-guard.py`** (added 2026-07-26, `docs/benchmarks/
  ...
  **The watchdog-vs-LOCKDOWN collision class, and its established
  mitigation:** any independent watchdog that restarts a service the
  thermal-ingest-guard also manages must check the guard's own tier
  before acting, or it will defeat a deliberate LOCKDOWN shed mid-emergency
  and page a misleading "auto-restarted" success notice for what was
  actually a defeated safety action. Root-scope `scripts/watchdog.sh` has
  carried the fix for its own managed containers (poller, pusher) since
  August: a `_guard_tier()` helper reads `thermal_ingest_guard_state.json`'s
  `tier` field directly (fails OPEN to 0 -- proceeds with the
  restart/page -- on a missing/unparseable state file, so a broken state
  file can never permanently mask a real outage) and suppresses
  restart+page at tier 2. `scripts/runner-health-watchdog.sh` (added
  2026-09-05, see ALERT_ARCHITECTURE.md/ALERT_REFERENCE.md) initially
  shipped WITHOUT this check -- caught the same day, before it ever hit
  a real LOCKDOWN, by an automated doc-drift pass on its own commit --
  and now ports the identical `_guard_tier()` pattern rather than
  inventing a second one. Any future watchdog for a guard-managed unit
