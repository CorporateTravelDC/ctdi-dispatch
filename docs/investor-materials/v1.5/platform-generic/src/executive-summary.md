...
# The economics: near-zero COGS against a five-to-six-figure subscription floor

New since the prior verification pass, and promoted to investor-facing at the founder's direction: a live-recomputed cost analysis (2026-08-24; re-derived live 2026-09-03, the source of every figure below) in which every commercial price cites a dated vendor source, business-judgment figures are flagged as assumptions, and vendors with no citable price anywhere are excluded from totals rather than guessed — a same-evening extension pass replaced the two largest former exclusions (FlightAware Firehose and Spire) with cited federal-procurement anchors.
...
| Recurring cloud-LLM spend | $0 — measured, not assumed: 35,217 logged LLM invocations over 56.9 days of continuous production contain zero cloud-model rows |
| Frontier-API counterfactual — the measured inference workload priced at cloud providers' published small-tier rates (hypothetical, not incurred; new in the 2026-09-03 pass) | ~$96-945 / yr |
| Defensible avoided cost, this instance, conservative | ~$2,160-2,656 / yr net |
| Subscription-replacement floor (purchasable subset of live capability) | ~$55,200-230,400 / yr |
| Live capabilities not purchasable at any price | 10 |

The one-line version: the platform runs on ~$765 of owned hardware and ~$22-38/yr of electricity, with $0 in data-feed fees and $0 in cloud-LLM spend, while the purchasable subset of its live capability lists at roughly $55k-230k/yr in commercial subscriptions. The high end roughly doubled in the 2026-09-03 extension pass for an honest reason: FlightAware Firehose and Spire, previously excluded as quote-only, now carry cited federal-procurement anchors (~$148k-224k/yr annualized from a U.S. Secret Service Firehose contract; ~$159k-200k/yr across three consecutive Navy Spire buys), while FAA SWIM access itself is confirmed $0 by the FAA's own stated policy. It remains a floor: ADS-B Exchange Enterprise and the SWIM-as-a-service intermediaries stay excluded after a documented exhausted search — and its most operationally valuable elements cannot be bought at any price: TFMS ground-stop/GDP/airspace-flow-program data, TBFM arrival metering, ITWS terminal wind-shear alerts, unfiltered (LADD-unbound) blocked-aircraft visibility from its own receivers, receive-side ACARS, a permanently-owned longitudinal corpus (commercial term licenses require destruction on exit), scheduled LLM briefs over a private multi-source corpus, corroborated entity auto-promotion, and the signed-manifest execution gates.

Honest boundaries, stated with the numbers as the source analysis requires: most of the ingested data is free at the source to any approved subscriber, so the avoided cost is an integration cost, not a data-license cost — the $55k-230k figure is a replacement-cost floor for a buyer without FAA vetting, and must not be presented as avoided spend (the honest avoided-spend number is the $2.2-2.7k/yr line, roughly two-thirds of which is reciprocal data-sharing barter rather than cash). Cost avoidance does not carry a valuation, and the analysis says so.

Two of the analysis's findings are core to the thesis rather than footnotes to it.

**Regulatory standing is the moat that never appears on an invoice.** The platform is an approved FAA SWIM subscriber under a signed, annually renewed Service Access Agreement; it operates the LADD compliance discipline that agreement family imposes — the SAA forbids displaying any LADD-listed aircraft and flows that duty down to every indirect consumer, which is why every commercial FAA-derived feed is LADD-filtered and paying more never unfilters it; and its own-RF reception sits lawfully outside LADD's contractual reach, by the FAA's own description of the broadcast. A buyer replicating this standing pays $0 in FAA fees — the data is free by the FAA's stated policy — but ~$48,600-145,900 in imputed professional labor (rates cited from GSA schedules; hours are flagged assumptions) across an FAA-stated 2-6 months of onboarding lead time, with the agreement renewed annually. And at the end of it they hold LADD-bound access: the blocked-aircraft visibility this platform has from its own receivers is not part of what they obtained.

**The second brain is the compounding asset, and its defining property is not purchasable.** A self-hosted vault of 9,579 indexed documents (live count, 2026-09-03), written to by seven automated units on live timers under a compiled semantic layer, permanently owned. Every commercial approximation is request-driven — a human asks, it answers; none runs scheduled synthesis over a private multi-domain operational corpus — and every one contractually takes the corpus back at exit (Guru deletes content 90 days after termination; Glean deletes customer data on termination; Smarsh may delete at its sole discretion six months after). The closest purchasable approximation runs ~$1,300/yr (a seat-minimum two-product composite, assumption-flagged) to ~$98,900/yr (the enterprise knowledge-platform median) — priced as approximation, not equivalence, and presented alongside, never inside, the replacement floor above.
...
- **A compounding knowledge layer.** The second brain accumulates operational memory (6,742 documents at the 08-24 snapshot) under a compiled concept graph with causal derivation edges — correlations invisible to any real-time view, growing daily by timer, and permanently owned where commercial knowledge platforms contractually delete the corpus at exit.
...
| Verification gaps | No dedicated CPS unit test; no CI; one known pre-existing test failure (tracked); several cost-analysis inputs are secondary-sourced; the Firehose/Spire high-end anchors are federal-procurement records with scope caveats, and a few equivalents (ADS-B Exchange Enterprise, SWIM-as-a-service intermediaries) remain quote-only after a documented exhausted search. |
