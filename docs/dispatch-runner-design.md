...
**Rewritten 2026-08-11 against `src/runner/main.py` (2383 lines), the live
...
| Public demo (playback) | `corporatetraveldc-runner-demo.service` | 8005→8001 | **Public**: `https://dispatch-runner.example.com` (CF tunnel → nginx → :8005), password-gated (`DEMO_MODE`), reads the demo API (:8004) instead of live feeds |
...
2. **Demo gate** (demo instance only): `DEMO_MODE=true` + untrusted origin ⇒
   ...
   (`DEMO_SANITIZE_SALT`).
3. **Tier-1 token injection**: for a small allowlist (`_TIER1_PATHS`:
   `api/v1/tfr-enriched`, `api/v1/radio`, `api/v1/cui/status`) the proxy
   injects the runner's `cert`-tier service token (`RUNNER_ENRICHED_TOKEN`);
   a client-supplied `Authorization` header always wins. Watchlist reads are
   deliberately NOT injected — dispatch-web gates them itself. See
...
shared with the second-brain RSS poller): **11 built-in categories, 27
