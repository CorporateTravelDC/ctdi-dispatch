...
> **2026-09-05 — `stdds_safety` is currently SILENCED, live.** The table
> above describes the code's fire behavior, which is unchanged. On top of
> it, an operator-applied standing decision (2026-09-04, raw/unconfirmed
> bitmask concern — see vault note `20260904T182619Z.md` §8) suppresses
> the actual push via `sector_coalesce.set_feed_silence("stdds_safety",
> True)`. `stdds_surface`/`stdds_taxi`/`stdds` (PCT) are NOT silenced.
> Check `/var/lib/corporatetraveldc/sector_coalesce_silence.json`
> (`silenced_feeds`) for current live state before assuming this table
> alone tells you whether stdds_safety is actually pushing right now —
> this is exactly the "deliberate silence looks identical to an outage"
> trap flagged elsewhere in this doc.
...
| `runner-health-watchdog.sh` | 4 on auto-restart, 5 + 30-min cooldown on failed restart | **Posts to `hot-alerts`, NOT `ops-health`** — the one exception to this table's "all post to ops-health unless noted" preamble. Added 2026-09-05. Both priorities relay via `X-Email` to the operator inbox. Guard-tier-aware since the same day (see GUARDRAILS_JUSTIFICATION.md/ALERT_ARCHITECTURE.md) — skips restart+page entirely at thermal-ingest-guard tier=2 (LOCKDOWN), since a not-active runner is then a deliberate shed, not an outage. Every 5 min. |
