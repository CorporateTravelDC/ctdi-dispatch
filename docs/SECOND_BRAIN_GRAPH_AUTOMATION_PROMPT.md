...
- The retrofit and builder are idempotent and incremental by design (the
  ...
  now holds **875** non-RSS notes
  (`sqlite3 /var/lib/corporatetraveldc/second_brain_index.db "SELECT COUNT(*) FROM vault_notes_fts WHERE path NOT LIKE '%/00-Inbox/rss/%';"`,
  read 2026-08-23). Re-derive rather than quoting a number here.)
