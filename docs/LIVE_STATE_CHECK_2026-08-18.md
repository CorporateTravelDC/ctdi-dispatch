...
## Addendum — ninth pass, 16:46–16:52 EDT, against HEAD `893b6b0` (delta only)

HEAD is `893b6b0` (16:45:53, "Add second-brain semantic layer; wire concept
edges into the knowledge graph"), 0 ahead of `origin/main`. 18 files: new
package `src/second_brain/semantic/` (`__init__`, `__main__`, `model`,
`compile`, `export`, `ontology.json`), new `scripts/second-brain-search.sh`,
new `tests/second_brain/test_semantic.py` (27 tests), plus edits to
`scripts/post-commit-doc-verify.sh` (+20/−7, prompt text only),
`src/second_brain/index_db.py` (`search_notes(raw=)` phrase-wrapping +
`--raw` CLI flag), `src/second_brain/knowledge_graph/build_graph.py`
(`_load_semantic()` overlay), the regenerated `graph.json`/`vault-graph.html`/
`viz_template.html`, `docs/SECOND_BRAIN_STATUS.md` (+80 addendum), and the
manifest pair. No unit, container, Modelfile, or topic touched. Nothing
staged, committed, or changed live by me; working tree = this file + the
operator's post-commit `MANIFEST.sha256.asc` (re-signed 16:46:33, 40 s after
the commit; `.sha256` itself is unchanged since the commit — same content,
fresh signature). `stash@{0}` from pass 8 is **still present**, still
droppable.

Second-brain pre-search (the new prompt asks for it; the hook that launched
this pass didn't — see drift 1): `scripts/second-brain-search.sh` finds the
16:47Z checkpoint and the 19:11Z "semantic layer built" note; no prior
LIVE_STATE pass has covered this area. Starting cold is correct here.

### What this commit changes live

- **Nothing in any running container.** The 06:26 `web 2eddb421694f` /
  `poller 2500e795979c` / `pusher 363961604403` images pre-date the commit:
  inside `poller:latest` there is **no** `src/second_brain/semantic/`, no
  `scripts/second-brain-search.sh`, and `index_db.py` lacks `raw: bool`
  (image manifest = 677 entries, self-consistent, so `verified-exec.sh` is
  unaffected). The only containerised consumer of the changed code is the
  04:00 `second-brain-index-scan` (`index_db --scan` in the poller image) —
  `--scan` never calls `search_notes()`, and no other `src/` module calls
  it either (grep: CLI-only), so the phrase-wrap fix has zero live effect
  until the next `build-images.sh`. `build_graph.py`'s overlay is guarded
  (`try: import … except ImportError: return {}, []`), so an old image
  running it would just build the pre-overlay graph.
- **The graph overlay IS live-served, without a rebuild:** the operator's
  16:19 build wrote `/var/lib/corporatetraveldc/knowledge_graph/{graph.json,
  vault-graph.html}` (byte-identical to the committed repo copies) and the
  `web` container bind-mounts that volume, so `/api/v1/knowledge-graph/{html,meta}`
  (T1-gated, `:8000` → 403 anonymous, as designed 2026-08-13) now serve
  meta `semantic_concept_nodes 96 / semantic_concept_edges 1869`,
  `node_count 641`, `edge_count 2734`, `isolated_notes 8`. Vault-side
  `04-Syntheses/vault-graph.html` exists (WebDAV-listed); whether the 16:19
  run used `--push-vault` I can't tell from a listing — same public-exposure
  posture as pass 2026-08-16 items 13/14, now carrying 96 ontology labels
  and definitions in addition to the note titles it already exposed
  (ontology content, not client data).
- **Semantic layer, verified against the live index (compiled 15:37 EDT):**
  9 `semantic_*` tables present in `second_brain_index.db`; exports
  `semantic_layer.{json,ttl}` + `SEMANTIC_LAYER.md` in
  `/var/lib/corporatetraveldc/semantic/` (15:37); `--stats` 5 facets / 96
  concepts (37 declared + 59 lexicon) / 412 surface forms; `--drift` **PASS**
  (rss_catalog superset, +149 forms, no stale entity_domains); `--metrics`
  `concept_coverage 0.8612`, `unmapped_tags 21`; the four retrieval-gain
  examples reproduce and have grown (rail 29→197, "EV tolls" 1→104,
  "executive protection" 37→112, AAM 24→104). `--semantic`/`--resolve`/
  `--expand`/`--concepts`/`--drift` modes of the search script all work
  with no credentials, as the header claims; the plain and `--raw` modes
  pick `NEXTCLOUD_ADMIN_USER` up from `dispatch.env`. **No HTTP/MCP surface**
  for it (grep of `src/web`, `src/runner`, `src/poller` → nothing) — matches
  the addendum's "open / not done".
- **Manifest:** contrary to the new addendum (drift 2 below), the commit
  itself signed everything in — **671 → 680 entries**, all 9 new paths
  present, `verify-manifest.sh` → **OK, 680 files** at 16:47. The
  `integrity-sweep` failure at **16:42:50** (7 mismatches incl.
  `viz_template.html`) is from the pre-sign edit window, 3 min *before* the
  commit — same benign class as passes 5/7/8; manual verify passes now, the
  16:57:49 fire should clear it.
- Tests: `tests/second_brain/` **27 passed** (0.26 s); full suite **17
  failed / 171 passed** (was 17/144 — +27 new, same 17 pre-existing
  failures).

### Drift found (this commit)

1. **The installed post-commit hook is stale — this very pass proves it.**
   `scripts/post-commit-doc-verify.sh` (3510 B, 14:14 today) now tells the
   agent to pre-search the second brain and to persist real findings via
   `remember_text()`; `.git/hooks/post-commit` is still the **2840 B copy
   from Aug 11 13:08** and differs from line 51 on. The prompt this session
   was launched with is the *old* wording (no second-brain paragraph). Pass
   7's "byte-identical" was true then and is not now; `docs/INFRA_MAP.md:179`
   ("`scripts/post-commit-doc-verify.sh`, installed as this repo's
   `post-commit` git hook") is now half-true — installed, but not this
   version. Not fixed by me: the hook lives under `.git/`, and the standing
   rule for these passes is hands off git plumbing. Operator fix is one
   line: `cp scripts/post-commit-doc-verify.sh .git/hooks/post-commit`.
   (`remember_text()` exists — `src/second_brain/remember.py:26` — so the
   new prompt's pointer is valid once installed.)
2. **`docs/SECOND_BRAIN_STATUS.md` 08-18 addendum, last paragraph:** "The
   new files are **not in `MANIFEST.sha256`** — the operator must run
   `scripts/sign-manifest.sh` … nothing breaks meanwhile" was written on the
   branch and is false as of the commit that carries it: the same commit's
   `MANIFEST.sha256` diff adds all 9 new paths (`+23` lines = 9 new + 7
   re-hashed pairs), verify OK. Stale from birth; harmless (it over-warns).
   The file is manifest-covered, so left as-is for an operator commit +
   re-sign rather than a doc-check edit.
3. **Graph-meta semantics changed under two docs that read the meta block:**
   - `docs/SECOND_BRAIN_STATUS.md:121-128` (08-11): "`build_graph.py` —
     nodes = curated notes (including hubs), edges = the actual
     `[[wikilinks]]`" — the graph now also carries **96 `concept` nodes**
     and **1,869 semantic edges** (`concept_tag` 1795, `in_domain` 59,
     `related_to` 10, `broader_than` 5) alongside 865 `wikilink` edges. The
     08-18 addendum documents the layer but never mentions the graph
     overlay, so the commit title's second half ("wire concept edges into
     the knowledge graph") is undocumented in the status doc — only
     `build_graph.py`'s own comment block covers it.
   - `docs/SECOND_BRAIN_GRAPH_AUTOMATION_PROMPT.md` steps 1 and 4 treat
     `node_count`, `edge_count`, `isolated_notes` as wikilink-structure
     numbers for week-over-week trends. As of this build `node_count` (641)
     = 545 notes/hubs + 96 concepts, `edge_count` (2734) is ~68 % semantic,
     and **`isolated_notes` = 8 now means "no edge of any kind"** — a note
     with only concept-tag edges no longer counts as isolated. Recomputed on
     wikilink edges alone, **314 of 545** note/hub nodes are isolated
     (prior snapshot 08-13: 29 of 254). The prompt's ISOLATED/HEALTH section
     would report 8 and call the trend improving; the wikilink signal it was
     written to track has to be derived from `edges[].type == "wikilink"`
     (or the `origin` field) instead of meta. Prompt is still DRAFT/unscheduled
     (no timer exists), so nothing is misreporting yet — but the first
     scheduled run would.
4. **Sub-threshold code nit, recorded not fixed:** `semantic/compile.py:302`
   reports `assignments = len(rows)` where `rows` is deduped on the full
   6-tuple (incl. `evidence`) but the table PK is `(path, concept_id, rule)`
   and the insert is `OR IGNORE` — live `semantic_meta.assignments = 40,253`
   vs `SELECT count(*) FROM semantic_note_concepts` = **39,744** (−509).
   The addendum's "40,134 assignments compiled" is the same over-count from
   an earlier compile. Reporting only; nothing consumes the meta figure.

### Still accurate (checked because this commit could have touched them)

- README.md, CLAUDE.md, `src/ingest/README.md`, `src/shared/watchlist_README.md`:
  no mention of the semantic layer, `index_db --search`, the graph builder,
  or the hook script. CLAUDE.md's "durable findings … `second_brain.remember
  --stdin --tags`" path is unchanged and still the write path the new prompt
  points at. **Nothing to update.**
- `docs/INFRA_MAP.md:349` (index_db → `second_brain_index.db`, daily 04:00
  scan): timer `active/waiting`, next 2026-08-19 04:00, last 04:00 today. ✅
- `docs/DEDICATED_MODELS_PLAN.md:114-115` (query `second_brain.index_db` FTS
  index): still valid; the new `--semantic` route is a strict superset, not a
  replacement. ✅
- `SECOND_BRAIN_STATUS.md` 08-18 addendum's other checkable claims — five
  facets/18 domains/9 genres/5 lifecycle/5 provenance/59 entities, drift
  PASS +149, credential-free import, exports in the shared volume, no HTTP/
  MCP surface, dispatch-mcp archived (already verified in pass 2026-08-17) —
  all ✅ live. Its "18 tags left in the governance backlog" is 21 now, and
  the three additions (`semantic-layer`, `ontology`, `taxonomy`) are the tags
  on the 19:11Z checkpoint note about this very work — growth, not drift.
- `sign-manifest.sh` carve-out (pass 7): appending this addendum does not
  break verification (re-run after write: OK, 680).

### Live state moved since the eighth pass (not from this commit)

- `podman ps` = **37**; core trio `Up 14 hours` on the 06:26 images; 21/21
  models; `:8000` 200, `:8001` 200. 118 units loaded; failed =
  `integrity-sweep` (16:42:50, transient, above) + `transport-pattern-digest`
  (12:53, unchanged). Load1 6.43 at 16:49.
- The 16:19 graph rebuild grew `curated_notes` **254 → 545** vs the 08-13
  snapshot committed at HEAD~1 — five days of daily-watch/checkpoint output
  plus the retrofit not having been re-run since; that, not the overlay, is
  why wikilink isolation is 314 (drift 3). Worth a retrofit pass before the
  trends prompt is ever scheduled.

**Bottom line (this pass): `893b6b0` invalidates nothing in README.md,
CLAUDE.md, or the two sub-READMEs, and its semantic-layer claims all
reproduce live. Three real doc drifts, none breaking anything today:
(1) `.git/hooks/post-commit` is the pre-change copy — every post-commit
doc-check until it is re-copied runs the old prompt (one `cp` fixes it);
(2) the new SECOND_BRAIN_STATUS addendum says the new files aren't in the
manifest — they are, since this commit; (3) `build_graph.py`'s meta
`node_count`/`edge_count`/`isolated_notes` now blend concept nodes/edges
into what STATUS.md and the (unscheduled) trends prompt describe as
wikilink-only figures — `isolated_notes 8` hides 314 wikilink-isolated
notes. Plus one reporting nit in `compile.py` (meta over-counts by 509).
Nothing live is wrong; the code is not in any container until the next
image build, and the served graph already reflects the overlay via the
shared volume.**
