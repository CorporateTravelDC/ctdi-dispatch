...
> **DATED SNAPSHOT — read the correction below before circulating this
> document.** The body of this file is a point-in-time summary written
> 2026-08-02 and last reverified 2026-08-11. It is deliberately preserved
> as written; nothing below this banner has been rewritten. One of its
...
> 2026-08-15:
>
> - `corporatetraveldc-runner-demo` has been crash-looping continuously
>   since 2026-08-15 (`NRestarts=49172` and climbing at this read) on a
>   `sqlite3.OperationalError: unable to open database file` at startup.
> - Its port `:8005` refuses connections, and the public hostname
>   `dispatch-runner.example.com` returns **HTTP 502**.
> - Separately, the password gate that section advertises is **not
>   configured** — `DEMO_MODE` is set in no environment file and no
>   Quadlet, so every demo-mode protection in `src/runner/main.py`
>   (including the `/api/demo/login` password gate) is inert. If the
>   crash were fixed without also setting `DEMO_MODE=true`, the result
>   would be an **ungated** public surface, not a password-gated one.
>
> Do not offer a demo link or access code on the strength of this
> document. See `CLAUDE.md` → "Core containers" for the open operator
> decision on whether this instance is meant to be public at all.
>
> The header's other claim — **all six FAA SWIM feeds provisioned and
> connected** — remains accurate. Note only that the feeds are
> deliberately stopped for parts of the day by
> `scripts/thermal-ingest-guard.py`'s load-shedding; a stopped ingest
> container is designed behaviour, not an outage.
