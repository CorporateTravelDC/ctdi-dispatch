...
>
> **2026-09-05 addition, and it's not alert-only:** `runner-health-watchdog.timer`
> (user-scope, every 5 min) also sits outside the layer model, but unlike
> the two guards above it has restart authority, not just publish
> authority -- it self-heals `corporatetraveldc-runner.service` before
> paging. Guard-tier-aware since the same day (skips both the restart
> and the page at thermal-ingest-guard tier=2/LOCKDOWN, where a
> not-active runner is a deliberate shed) -- see GUARDRAILS_JUSTIFICATION.md.
>
...
> **2026-09-05 — `stdds_safety` is currently SILENCED, live**, via
> `sector_coalesce.set_feed_silence("stdds_safety", True)` (operator
> decision, 2026-09-04, raw/unconfirmed bitmask concern — see vault note
> `20260904T182619Z.md` §8). The table above still correctly describes
> the code's fire behavior; the silence flag suppresses the push on top
> of it. `stdds_surface`/`stdds_taxi`/`stdds` (PCT) are unaffected. See
> `docs/ALERT_REFERENCE.md`'s STDDS table for the same note in more
> detail.
