...
# -4 on every Cloudflare API call below is REQUIRED, not stylistic.
# CF_MANAGEMENT_API_TOKEN is IP-filtered to this box's two PUBLIC IPv4 uplink
# egress addresses (wld0 / enu1), and this host prefers IPv6. Without -4 the
# request leaves from the v6 address, misses the allowlist, and Cloudflare
# returns code 1000 "Invalid API Token" -- byte-identical to a revoked
# credential. That false signal cost a full misdiagnosis on 2026-09-22
# (concluded the token was dead and rotated a perfectly good one). Verified
