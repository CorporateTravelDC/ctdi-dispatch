...
#
# --stagger=Ns   MINIMUM seconds each unit gets before the next one starts
#                (default 60 = SERIAL_BRINGUP_MIN_WINDOW_S). Since 2026-09-06
#                start/restart go through scripts/lib/serial-bringup.sh:
#                after each unit, load1 is sampled every 10 s and the next
#                unit waits until the window has elapsed AND load1 <
#                SERIAL_BRINGUP_LOAD_MAX (12), capped at
#                SERIAL_BRINGUP_MAX_WAIT_S (300) per unit. Was a flat sleep
#                of 15 s. Stop always fires everything at once (no reason
#                to delay freeing resources).
...
do_start_or_restart() {
    ...
    echo "${action}ing (order=${order}, min window=${stagger}s, load gate < ${SERIAL_BRINGUP_LOAD_MAX}, cap ${SERIAL_BRINGUP_MAX_WAIT_S}s): ${targets[*]}"
    # Serialized + load-gated (2026-09-06, see --stagger in the header):
    # the library samples load1 every 10 s after each unit and holds the
    # next one until the window has passed AND load1 is under the gate.
    local -a units=()
    for t in "${targets[@]}"; do units+=("$(unit_name "${t}"):${stagger}"); done
    serial_bringup_run "${action}" "${units[@]}"
}
