...
**Serialized, load-gated restore (2026-09-06, operator directive,
work-order item 1).** A LOCKDOWN restore used to bring the whole stack back
through ingest-feed-ctl.sh's flat 15 s stagger plus three back-to-back
`systemctl start`s -- itself a restart storm (peak load 40.28 for a flat
restart of this stack that morning). The restore now runs the same
discipline scripts/serialized-rollout.sh proved live the same day (peak
9.61): scripts/lib/serial-bringup.sh brings ONE unit up at a time, in
boot order (SWIM feeds lightest-first, ingest-core, poller, pusher,
runner), samples load1 every 10 s and holds the next unit until that
unit's window has elapsed AND load1 < SERIAL_BRINGUP_LOAD_MAX, hard-capped
per unit at SERIAL_BRINGUP_MAX_WAIT_S. The bash library is called rather
than re-implemented here because the guard already treats bring-up as an
external shell primitive (feed_ctl -> ingest-feed-ctl.sh, _user_unit_ctl
-> systemctl) and one implementation of the gate must serve boot, the
...
SERIAL_BRINGUP = "/opt/corporatetraveldc/private/ctdi-dispatch-internal/scripts/lib/serial-bringup.sh"
...
# Restore order = boot order (stack-boot-ctl.sh ORDER / ingest-feed-ctl.sh
# LIGHT_ORDER): SWIM feeds lightest-first, ingest-core, then the user units.
# "unit:window" = that unit's minimum window, values from
# serialized-rollout.sh's live run.
LOCKDOWN_RESTORE_INGEST = [
    "corporatetraveldc-ingest-notam", "corporatetraveldc-ingest-itws",
    "corporatetraveldc-ingest-tbfm", "corporatetraveldc-ingest-tfms",
    "corporatetraveldc-ingest-stdds", "corporatetraveldc-ingest-fdps:90",
    "corporatetraveldc-ingest-core:90",
]
LOCKDOWN_RESTORE_USER = ["corporatetraveldc-poller:180", "corporatetraveldc-pusher", "corporatetraveldc-runner:90"]
...
def _serial_env(cfg):
    """Environment for a serial-bringup child: the SERIAL_BRINGUP_* knobs
    from dispatch.env (this process does not source it) on top of our own.

    Observe-only thermal monitor (operator decision 2026-09-06): while the
    serialized restore runs, this guard's timer cannot fire, so the library
    reads the CPU temp at every sample and logs a WARNING at/above our
    tier-2 threshold -- it never acts on it (that stays this guard's job,
    on its next fire). Forward tier-2 so both use the same number."""
    env = dict(os.environ)
    cfg = cfg or {}
    env.setdefault("SERIAL_BRINGUP_TEMP_WARN_C",
                   str(int(_float(cfg.get("THERMAL_GUARD_TIER2_TEMP_C"), 79.0))))
    env.update({k: v for k, v in cfg.items() if k.startswith("SERIAL_BRINGUP_")})
    return env


def _serial_budget(cfg, n_units):
    """Wall-clock budget for a serial bring-up of n units: per unit, the
    library's pre-start hold and post-start window are each capped at
    SERIAL_BRINGUP_MAX_WAIT_S, so 2*cap per unit is the true worst case."""
    cap = _float((cfg or {}).get("SERIAL_BRINGUP_MAX_WAIT_S"), 300)
    return int(n_units * 2 * cap + 60)


def feed_ctl(action, target, cfg=None):
    ...
    if action in ("start", "restart"):
        # 2026-09-06: start/restart are serialized + load-gated inside
        # ingest-feed-ctl.sh now (scripts/lib/serial-bringup.sh), so the
        # budget scales with the library's per-unit cap, and its per-unit
        # spike annotations go to our journal instead of being discarded.
        budget = max(budget, _serial_budget(cfg, n))
        sys.stdout.flush()  # keep our journal lines ordered before the child's
        try:
            subprocess.run(
                [FEED_CTL, action, target],
                check=False, text=True, timeout=budget, env=_serial_env(cfg),
            )
        except Exception as e:
            print(f"{LOG_PREFIX} feed_ctl {action} {target} did not finish cleanly: {e}")
        return
    subprocess.run(
        [FEED_CTL, action, target],
...
def _serial_bringup(cfg, action, units):
    """Bring `units` (name[:window] specs, in order) up ONE at a time,
    load-gated, via scripts/lib/serial-bringup.sh -- output flows to the
    journal (that is the point: baseline/peak/end per unit). Best-effort,
    never raises; verification happens separately via
    _lockdown_stack_down_list(), same discipline as _user_unit_ctl()."""
    sys.stdout.flush()  # keep our journal lines ordered before the child's
    try:
        subprocess.run(
            [SERIAL_BRINGUP, action] + list(units),
            check=False, text=True, timeout=_serial_budget(cfg, len(units)),
            env=_serial_env(cfg),
        )
    except Exception as e:
        print(f"{LOG_PREFIX} serial bring-up ({action} {' '.join(units)}) did not finish cleanly: {e}")
...
def _lockdown_start_stack(cfg):
    """2026-09-06: serialized, load-gated, in boot order (see the module
    docstring's "Serialized, load-gated restore" paragraph). Was
    feed_ctl("restart","all") + feed_ctl("restart","core") + three flat
    starts. Same actions as before per unit -- restart for the ingest
    units, start for poller/pusher/runner -- one unit at a time."""
    _serial_bringup(cfg, "restart", LOCKDOWN_RESTORE_INGEST)
    _serial_bringup(cfg, "start", LOCKDOWN_RESTORE_USER)
    ...
    if tier > 0:
        ...
        if temp_ok and load_ok:
            ...
            if now - below_since >= resume_dwell:
                ...
                # 2026-09-06: both restores are serialized + load-gated
                # (scripts/lib/serial-bringup.sh) and can take many
                # minutes; `now` is stale by the time they return.
                if tier == 1:
                    restored_desc = tier1_feeds
                    feed_ctl("restart", tier1_feeds, cfg)
                    time.sleep(5)
                    ...
                    _lockdown_start_stack(cfg)
                    time.sleep(5)
                    failed = _lockdown_stack_down_list()
                restore_done = time.time()
                ...
                if failed:
                    ...
                    # 2026-09-06: stamped when the LAST unit is up, not when
                    # the restore began -- the serialized bring-up can run
                    # for many minutes, and the dormant window
                    # (THERMAL_GUARD_RESTORE_DORMANT_S) must cover the
                    # settle-in of the final unit, not expire mid-sequence.
                    state = {"tier": 0, "below_resume_since": None, "restored_at": restore_done,
                             "restored_via": "guard"}
            save_state(state)
