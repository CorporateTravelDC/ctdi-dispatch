...
# -- 7b. env values that must never be quoted (2026-09-21) -------------------
# dispatch.env is read by FOUR parsers that disagree about quoting, and
# podman --env-file is the one that cannot tolerate quotes -- it passes them
# through literally, so a quoted value silently stops matching at the
# consumer (poller/fetchers/amtrak.py) with no error anywhere. This has been
# "helpfully fixed" by adding quotes more than once. The full reasoning lives
# beside the value in config/dispatch.env; this check is what makes it stick.
#
# Deliberately checks BOTH the tracked copy and the live /etc copy: they
# drift independently, and the live one is what podman actually reads.
for _f in "${REPO_DIR}/config/dispatch.env" /etc/corporatetraveldc/dispatch.env; do
    [[ -r "${_f}" ]] || continue
    while IFS= read -r _line; do
        drift "quoted value in ${_f} -- podman --env-file passes quotes through literally, breaking the consumer: ${_line}"
    done < <(grep -nE '^(AMTRAK_CORE_ROUTES|AMTRAK_WATCHLIST_STATIONS|AMTRAK_REGIONAL_STATIONS)=["'"'"']' "${_f}" 2>/dev/null)
done
