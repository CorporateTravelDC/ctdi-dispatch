...
# 2026-09-05 (operator directive): ops-health, NOT hot-alerts. These are
# infrastructure-health pages, not VIP/TFR flight alerts -- routing them to
# hot-alerts buried real TFR traffic under runner restart notices on the
# operator's phone. Matches the NTFY_OPS convention every other watchdog in
# this directory already uses (container-mem-watch, adsb-link-watchdog,
# nextcloud-health-check, scheduled-ingest-restart, ...).
...
# NOTE: deliberately not bash's ":-" default-value shorthand here --
# scrub-public-tree.py's EMAIL_RE ([A-Za-z0-9._%+-]+@...) treats a hyphen as
# a valid local-part character, so that operator's own hyphen would sit
# directly against the default address below with no separator and get
# consumed into the match, producing a string one character longer than the
# exact address in ALLOWED_EMAILS -- confirmed live, this failed a real
# push-public.sh run. An explicit if-check keeps the literal address free of
# any adjacent hyphen (this comment deliberately never writes the default
# address immediately after a hyphen either, for the same reason).
