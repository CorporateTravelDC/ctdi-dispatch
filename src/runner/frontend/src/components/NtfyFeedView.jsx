...
function IntegrationsPanel() {
  ...
  // Tracks real component unmount (not interval-rearm) so an in-flight
  // fetch from just before unmount can't setState after the fact -- same
  // guarantee the old inline `cancelled` closure gave, just scoped to the
  // component's lifetime instead of one setInterval's.
  ...
  const poll = useCallback(async () => {
    try {
      ...
      if (!unmountedRef.current) setLog(data)
    } catch { /* transient -- keep last known state */ }
  }, [])

  useVisibilityAwareInterval(poll, 4000)
