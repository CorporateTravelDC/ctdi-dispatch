...
async def chat_history_clear(request: Request):
    ...
    if not _is_trusted(request):
        return JSONResponse(status_code=404, content={"detail": "Not Found"})
    await asyncio.to_thread(_chat_clear)
...
# ── Persistent dispatch chat history ────────────────────────────────────────
# 2026-09-20 Postgres cutover: this used to be its own standalone SQLite
# file (dispatch-chat.db, opened directly via sqlite3.connect(CHAT_DB_PATH)
# -- never routed through common.db.conn(), so it sat outside both the
# original 65-table write-path cutover and the (now-empty)
# REFERENCE_TABLES set). Routes through the same db.conn() the rest of
# the platform uses: chat_messages lives in
# src/common/pg_schema/0056_dispatch_chat.sql on Postgres (applied via
# scripts/pg_migrate.py, same as the other 55 files) and in
# common.db.SCHEMA for the SQLite fallback path. Confirmed live: 0 rows
# in both the prod and demo dispatch-chat.db mounts pre-cutover, so no
# bulk-copy was needed -- the table just starts empty on both engines.
from common import db

def _chat_db_init() -> None:
    """Ensure chat_messages exists. Real DDL on the SQLite fallback path
    (db.init_db()'s executescript(SCHEMA)); a confirmed no-op on Postgres,
    where the table is already built by 0056_dispatch_chat.sql -- see
    db_backend._TranslatingConnection.executescript()."""
    db.init_db()
...
def _chat_clear() -> None:
    """Erase all chat history from the persistent store."""
    with db.conn() as c:
