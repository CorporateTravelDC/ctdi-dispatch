...
# 2026-08-23: load-attributed-fallback event log, read by
# scripts/thermal-ingest-guard.py to fold "how many briefs just fell back
# to deterministic because Ollama was genuinely contended" into its own
# shed decision, alongside raw load1. Deliberately narrow attribution --
# see _record_load_fallback()'s docstring for exactly which failures
# count and which don't (a deliberately-stopped Ollama, e.g. tonight's
# 2026-08-22/23 incident, must NOT count here, or ingest would shed
# itself every time an operator takes Ollama down on purpose).
...
def _record_load_fallback(reason: str) -> None:
    """Append one event to LOAD_FALLBACK_LOG. Best-effort, never raises --
    matches the sr1_log.log_usage() convention (a logging side-channel
    must never be able to break the actual call it's observing).

    Called only for failures that are genuinely attributable to Ollama
    being CONTENDED, not merely unavailable:
      - OllamaBusyError ("slot unavailable") -- the client-side slot lock
        couldn't be acquired because something else has it. Unambiguous
        contention signal by construction.
      - httpx.TimeoutException (and subclasses: ConnectTimeout/ReadTimeout/
        WriteTimeout/PoolTimeout) from the actual generate call -- the
        server was too slow to respond in time, consistent with real
        contention.
    Deliberately NOT called for httpx.ConnectError or any other
    transport failure -- those mean Ollama is genuinely not there
    (stopped, crashed, wrong port), which is an availability problem, not
    a load problem. Conflating the two would make thermal-ingest-guard.py
    shed ingest every time an operator deliberately stops Ollama (exactly
    what happened 2026-08-22/23 -- see CLAUDE.md's incident writeup) --
    the one scenario this attribution scheme exists to avoid.
    """
    try:
        with open(LOAD_FALLBACK_LOG, "a") as f:
...
def _ollama(
    ...
    try:
        ...
        # Server was too slow to respond in time -- consistent with real
        # contention, unlike a connect failure (server not there at all,
        # handled by the generic branch below). See _record_load_fallback()'s
        # docstring for why this attribution split matters.
        log.warning("llm: Ollama call timed out: %s", exc)
        _record_load_fallback("generation_timeout")
        return None
    except Exception as exc:
        ...
        # Deliberately NOT calling _record_load_fallback() here -- this
        # branch is dominated by httpx.ConnectError (Ollama genuinely not
        # there), which is an availability problem, not a load one.
