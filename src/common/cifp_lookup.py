...
2026-09-19: cifp_fixes/cifp_holds/cifp_procedure_legs moved from the
SQLite-only reference set to Postgres (docs/POSTGRES_MIGRATION.md sec2's
JOIN exception -- geometric reasoning needs to JOIN these against
flight_events). This module's 5 connection sites are db_backend.pg_conn()
now; briefly, earlier the same day, they were db_backend.ref_conn() as an
