...
def prune_audit_log(days: int = 90) -> int:
    """Delete audit_log rows older than `days`. Returns rows deleted.

    2026-08-19: added alongside require_admin's new per-action audit
    writes (see auth.py) -- without this, audit_log grows unbounded
    forever, the same gap flight_events had before its 2026-07-27
    retention fix. Invoked daily by poller/skills/audit_log_prune.py.
    """
    cutoff = time.time() - days * 86400
    with conn() as c:
        cur = c.execute("DELETE FROM audit_log WHERE event_time < ?", (cutoff,))
        return cur.rowcount
