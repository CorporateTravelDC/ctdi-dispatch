...
def conn() -> Generator[sqlite3.Connection, None, None]:
    ...
    DISPATCH_DB_BACKEND=postgres (Postgres migration Phase 1, see
    docs/POSTGRES_MIGRATION.md §4): delegates to common.db_backend.pg_conn()
    -- a psycopg_pool connection with the same commit-on-exit/rollback-on-
    exception contract, dict rows, and a `?`/`:name` -> `%s`/`%(name)s`
    placeholder shim so nothing below this line changes. This is the ONLY
    branch point; every one of this module's 500+ call sites is untouched.
    DISPATCH_DB_BACKEND defaults to (and, absent an explicit operator
    change, stays) sqlite -- the branch below is today's behaviour,
    ...
    if db_backend.backend() == "postgres":
        with db_backend.pg_conn() as pgc:
            yield pgc
        return
...
-- Dispatch chat history (2026-09-20 cutover: was runner/main.py's own
-- standalone dispatch-chat.db SQLite file -- see
-- src/common/pg_schema/0056_dispatch_chat.sql for the Postgres side)
...
def init_db_all() -> None:
    """Run every init_db()/init_db_vN() function in this module, in
    version order, against the current DB connection. Added 2026-08-20 as
    the auto-detecting counterpart to web/main.py, poller/main.py, and
    ingest/main.py's own hand-listed init_db_vN() call chains -- those
    three are deliberately curated per-service subsets (ingest in
    particular intentionally skips versions it has no tables for, see its
    own startup comment) and this function does NOT replace them.

    This exists for callers that want the FULL current schema with zero
    chance of drifting stale, the same anti-hardcoding principle CLAUDE.md
    already applies to container/unit counts ("don't hardcode a total
    count, check live state instead") -- applied here to schema versions.
    Root cause this fixes: two test fixtures (tests/shared/test_watchlist.py's
    _IsolatedDB, tests/web/test_watchlist_batch.py's _setup_test_db) each
    hand-maintained their own partial copy of which migrations to apply --
    one stopped at init_db_v5() (missing hex_id/registration, added in
    v18), the other hand-copied a CREATE TABLE string that just fell
    further behind over time. Introspection makes that whole failure mode
    structurally impossible: any future SCHEMA_Vn is picked up automatically,
    nothing to remember to update.

    Safe to call repeatedly/out of order -- every init_db_vN() is itself
    idempotent (CREATE TABLE IF NOT EXISTS / ALTER TABLE with a
    ...
    for name in dir(this_module):
        m = _re.fullmatch(r"init_db(?:_v(\d+))?", name)
        if not m:
            continue
        fn = getattr(this_module, name)
        if not callable(fn):
            continue
        version = int(m.group(1)) if m.group(1) else 1  # bare init_db() sorts first
        candidates.append((version, name, fn))

    for _version, _name, fn in sorted(candidates, key=lambda t: t[0]):
        fn()
...
def _ensure_pull_path_status(c) -> None:
    # 2026-09-19: no-op on Postgres -- schema is already fully built by
    # common/pg_schema/*.sql migrations there. This is a lazy "ensure the
    # table exists" DDL statement re-run on EVERY call (upsert_pull_path_
    # status, get_pull_path_status), not a one-time init_db_vN() -- the
    # SQLite-only init_db_vN() postgres guard added earlier tonight doesn't
    # cover this pattern. Silent no-op even where the DDL is syntactically
    # valid on both engines: pure wasted round-trips against Postgres on
    # every single call otherwise. See _ensure_board's sibling comment for
    # the case (AUTOINCREMENT) where the DDL isn't even valid Postgres
    # syntax -- confirmed live, that one 500'd every /api/v1/board* call.
    from common import db_backend
    if db_backend.backend() == "postgres":
        return
    c.execute(
...
def _ensure_board(c) -> None:
    # 2026-09-19: no-op on Postgres. `seq INTEGER PRIMARY KEY AUTOINCREMENT`
    # below is not valid Postgres syntax at all (Postgres has no
    # AUTOINCREMENT keyword -- SERIAL/GENERATED ... AS IDENTITY instead),
    # so this failed with a hard syntax error on EVERY call to board_query/
    # board_threads/board_insert -- confirmed live, 500 on every /api/v1/
    # board* endpoint after the cutover. The table already exists on
    # Postgres via common/pg_schema/*.sql; this lazy ensure-on-every-call
    # pattern (not a one-time init_db_vN(), so the earlier postgres guard
    # didn't cover it) was always redundant there anyway.
    from common import db_backend
    if db_backend.backend() == "postgres":
        return
    c.execute(
...
def board_insert(from_side: str, to_side: str, thread: str, subject: str,
    ...
    with conn() as c:
        ...
        # .lastrowid is SQLite-only (sqlite3.Cursor has it, psycopg's doesn't) --
        # RETURNING + fetchone() is the one pattern both engines support, and
        # row_factory is sqlite3.Row / dict_row on the two backends respectively,
        # so key access (not positional) works on both.
...
def _ensure_board_auth(c) -> None:
    # 2026-09-19: no-op on Postgres -- see _ensure_board's sibling comment.
    # Schema already fully built there; this lazy per-call ensure is
    # redundant (and, unlike _ensure_board, syntactically valid on both
    # engines here, so purely wasted round-trips rather than a hard
    # failure -- still worth skipping for the same reason).
    from common import db_backend
    if db_backend.backend() == "postgres":
        return
    c.execute(
...
def upsert_nas_program(program_id: str, prog_type: str, facility: str,
                       raw_json: str, key_scheme: int | None = None,
                       legacy_correlate_id: str | None = None) -> None:
    """key_scheme/legacy_correlate_id: see SCHEMA_V36 / init_db_v36()'s
    docstring. Both default to None (not applicable) for callers that
    don't pass them -- the REST poller/fetchers/nas.py path and any AFP
    write never set these, only write_tfms_programs()'s GDP/GS branch
    does. COALESCE on update so a later call that omits these (there
    isn't one today, but nothing guarantees that forever) can't blank out
    a value a previous call already set."""
    with conn() as c:
        c.execute("""
            INSERT INTO nas_programs
                (program_id, type, facility, raw_json, active, fetched_at,
                 key_scheme, legacy_correlate_id)
            VALUES (?, ?, ?, ?, 1, unixepoch(), ?, ?)
            ON CONFLICT(program_id) DO UPDATE SET
                ...
                active=1,
                fetched_at=excluded.fetched_at,
                key_scheme=COALESCE(excluded.key_scheme, nas_programs.key_scheme),
                legacy_correlate_id=COALESCE(excluded.legacy_correlate_id, nas_programs.legacy_correlate_id)
        """, (program_id, prog_type, facility, raw_json, key_scheme, legacy_correlate_id))


def find_legacy_nas_program(prog_type: str, facility: str,
                            within_hours: float = 48.0) -> str | None:
    """Best-effort correlation for the key_scheme cutover (SCHEMA_V36):
    the most recent still-key_scheme=1 nas_programs row for the same
    type+facility, fetched within `within_hours` of now. GDP/GS programs
    are typically hours-to-a-day in duration, so a same-type/same-airport
    row seen recently is very likely the pre-fix identity of the program
    now being written under the new key -- but this is a proximity
    heuristic, not a guaranteed exact match (two genuinely distinct GDPs
    at the same airport within the window would also match). Returns
    None if nothing qualifies -- the caller (write_tfms_programs) treats
    that as "no correlation found", not an error."""
    with conn() as c:
        row = c.execute("""
            SELECT program_id FROM nas_programs
            WHERE type = ? AND facility = ? AND key_scheme = 1
              AND fetched_at >= unixepoch() - ?
            ORDER BY fetched_at DESC LIMIT 1
        """, (prog_type, facility, within_hours * 3600)).fetchone()
...
def audit_count_24h() -> int:
    # 2026-09-19: `row[0]` positional indexing found live on Postgres --
    # dict_row rows (the postgres row_factory, see db_backend.py) are plain
    # dicts, no positional access. sqlite3.Row supports both, which is
    # exactly why this went undetected until this specific code path first
    # ran against Postgres (confirmed live: KeyError: 0 from /healthz).
    # Explicit `AS n` + name-keyed access works on both engines.
    cutoff = time.time() - 86400
    with conn() as c:
        row = c.execute(
            "SELECT COUNT(*) AS n FROM audit_log WHERE event_time >= ?", (cutoff,)
        ).fetchone()
        return row["n"] if row else 0


def prune_audit_log(days: int = 90) -> int:
    """RETIRED 2026-09-22 -- refuses to delete. Always raises.

    Was: a bare DELETE of audit_log rows past `days`, called daily by
    poller/skills/audit_log_prune.py (now a stub -- see that file for the
    full rationale).

    The operator retention decision (2026-09-21) is that audit rows are
    never destroyed, only checkpointed, signed, moved off-box, and reduced
    on-device to a signed stub. Destroying an audit row with no signed
    successor is the one outcome that cannot be undone or reconstructed,
    so this raises rather than returning 0: a silent no-op would let a
    caller believe retention was being enforced while the table grew
    unbounded, and would leave a live DELETE path one call away from any
    future code that found this name in the docs.

    Retention now runs through poller/skills/audit_log_archive.py.
    """
    raise RuntimeError(
        "prune_audit_log() is retired: audit rows are archived and signed, "
        "never deleted in place. Use poller/skills/audit_log_archive.py "
        "(horizon: AUDIT_ARCHIVE_RETENTION_DAYS)."
    )
...
def mark_audit_egress_failed(audit_id: int, error: str, retry_limit: int = 5) -> None:
    ...
    with conn() as c:
        ...
        attempts = (row["egress_attempts"] if row else 0) + 1
...
def set_token_department(token_prefix: str, department: str | None) -> int:
    ...
    with conn() as c:
        # 2026-09-20: `SELECT changes()` is SQLite-only syntax -- doesn't
        # exist on Postgres at all (a query error, not just a wrong value).
        # The mutating statement's own cursor already carries rowcount on
        # both engines (same pattern as prune_audit_log() above).
        cur = c.execute("""
...
def revoke_token(token_prefix: str) -> int:
    ...
    with conn() as c:
        # 2026-09-20: same `SELECT changes()` -> cursor.rowcount fix as
        # set_token_department() above -- SQLite-only syntax, doesn't
        # exist on Postgres.
        cur = c.execute("""
...
def get_protected_flight_ids() -> set[str]:
    ...
    with conn() as c:
        idents: set[str] = {
            row["identifier"] for row in c.execute(
                "SELECT identifier FROM watchlist_entries WHERE entry_type='flight'"
            ).fetchall()
        }
        idents |= {
            row["subject"] for row in c.execute(
        ...
        for ident in idents:
            ...
            protected |= {r["flight_id"] for r in rows}
        return protected
...
def record_feed_bytes(feed_name: str, bytes_in: int,
    ...
    with conn() as c:
        c.execute("""
            ...
            ON CONFLICT(feed_name) DO UPDATE SET
                bytes_in = feed_data_usage.bytes_in + excluded.bytes_in,
                records_in = feed_data_usage.records_in + excluded.records_in,
                records_accepted = feed_data_usage.records_accepted + excluded.records_accepted,
    ...
    return [
        {
            ...
            "records_dropped": max(0, r["records_in"] - r["records_accepted"]),
            "filter_pass_pct": round(100 * r["records_accepted"] / r["records_in"], 1) if r["records_in"] else None,
...
def extend_watchlist_auto_remove_for_delay(entry_id: str, new_auto_remove_at: str,
                                           ...
                                           scheduled_arrival: str | None = None) -> bool:
    """Extends (and, if the entry started on the added-time fallback
    because scheduled_arrival wasn't known yet, first corrects onto the
    real schedule) a transient flight watchlist entry's auto_remove_at
    once it's gone airborne -- see
    shared.watchlist.extend_auto_remove_for_delay() for the full
    rationale. `WHERE departure_delay_min IS NULL` is the once-only
    guard (SCHEMA_V37) -- TFMS can resend the same airlineOffTime on a
    later message, and delay_min may legitimately be 0 (on-time/early
    departure); either way this only ever applies once per entry.
    scheduled_departure/scheduled_arrival are COALESCEd, not overwritten
    -- never clobber a real value the entry already had. Returns True if
    the row was actually updated (False if already applied)."""
    with conn() as c:
        cur = c.execute("""
            UPDATE watchlist_entries
            SET auto_remove_at = :new_auto_remove_at,
            ...
            WHERE id = :id AND departure_delay_min IS NULL
        """, {
            "id": entry_id,
            "new_auto_remove_at": new_auto_remove_at,
            "delay_min": delay_min,
...
def get_tbfm_sequence_for_flight(flight_id: str, max_age_seconds: float = 1800) -> dict | None:
    """Freshest tbfm_sequences row for one flight (TBFM's own `aid`
    ACID/callsign key -- NOT an FDPS GUFI, see fdps_parser.py's
    write_flight_event() for the join, which correctly uses callsign, not
    its own flight_events.flight_id, since TBFM never carries a GUFI).
    None if no row, or the freshest row's last_seen is older than
    max_age_seconds (same 1800s/30min staleness bar as
    get_active_tbfm_sequence_count() -- a flight that's left the metering
    queue shouldn't keep contributing a now-stale ETA)."""
    flight_id = (flight_id or "").strip().upper()
    if not flight_id:
        return None
    with conn() as c:
        row = c.execute(
            """SELECT * FROM tbfm_sequences WHERE flight_id=?
    ...
    try:
        age = (datetime.utcnow()
               - datetime.strptime(row["last_seen"], "%Y-%m-%dT%H:%M:%SZ")).total_seconds()
    except ValueError:
        return None
...
def get_tbfm_sequences_for_flight(flight_id: str,
                                  max_age_seconds: float = 1800) -> list[dict]:
    """ALL fresh tbfm_sequences rows for one flight, freshest first.

    2026-09-05: added because get_tbfm_sequence_for_flight()'s LIMIT 1 is
    actively wrong for the arrival_time enrichment. A single flight
    routinely carries MANY concurrent sequence rows (UAL1635 had 42), and
    TBFM mixes real fixes (CAVLR, MULRR, BILIT, HVQ, ...) with
    facility/meter-point PSEUDO-LABELS (ZDC, DC_MET, IAD_MP, HVQ_BKW,
    J48_J75, ZID-E, ...) that have no coordinates anywhere. Taking simply
    the freshest row means a pseudo-label usually wins, resolve_fix()
    returns None, and the enrichment silently produces nothing -- which is
    exactly what happened in production (0 populated rows; the freshest
    row for UAL1635 was the pseudo-label 'ZDC').

    Callers needing real geometry should walk this list and take the first
    entry whose meter_fix actually resolves, rather than assuming the
    newest row is usable. Same 1800s staleness bar as the singular
    version."""
    flight_id = (flight_id or "").strip().upper()
    if not flight_id:
        return []
...
def upsert_tbfm_sequence(meter_fix: str, facility: str, flight_id: str,
                         ...
                         eta_kind: str | None = None) -> None:
    """eta_kind (v46, 2026-09-05): which arc the ETA came from --
    "rwy"/"mfx"/"dfx"/"sfx", or None when unknown. Keyword-with-default so
    existing callers and tests keep working; a None here means UNKNOWN and
    must never be inferred as "mfx" downstream. See init_db_v46()."""
    now = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
    with conn() as c:
        c.execute("""
            INSERT INTO tbfm_sequences
                (meter_fix, facility, flight_id, eta, sequence_num, assigned_speed,
                 last_seen, eta_kind)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def osint_add_scope(label: str, scope_type: str, query_terms: str,
    ...
    with conn() as c:
        ...
        # .lastrowid is SQLite-only -- see board_insert()'s comment above.
...
def faa_ladd_count() -> int:
    with conn() as c:
        row = c.execute("SELECT COUNT(*) AS n FROM faa_ladd_aircraft").fetchone()
        return row["n"] if row else 0
...
def _safe_mark_and_sweep(table: str, cutoff_epoch: float) -> int:
    ...
    with conn() as c:
        existing = c.execute(f"SELECT COUNT(*) AS n FROM {table}").fetchone()["n"]
        if existing == 0:
            return 0
        survivors = c.execute(
            f"SELECT COUNT(*) AS n FROM {table} WHERE updated_at >= ?", (cutoff_epoch,)
        ).fetchone()["n"]
        if survivors == 0:
            log.error(
...
def faa_registry_meta_get(key: str) -> str | None:
    # 2026-09-20: `row[0]` positional access -- same dict_row-has-no-
    # positional-index bug as audit_count_24h() above. The queried column
    # is already named `value`, so name-keyed access needs no `AS` alias.
    with conn() as c:
        row = c.execute(
...
def faa_registry_count() -> dict:
    # 2026-09-20: same fix as faa_registry_meta_get() above -- `AS n` +
    # name-keyed access instead of positional `[0]`.
    with conn() as c:
        total = c.execute("SELECT COUNT(*) AS n FROM faa_aircraft_registry").fetchone()["n"]
        valid = c.execute(
            "SELECT COUNT(*) AS n FROM faa_aircraft_registry WHERE status_code='V'"
        ).fetchone()["n"]
        ladd  = c.execute("SELECT COUNT(*) AS n FROM faa_ladd_aircraft").fetchone()["n"]
...
def opensky_registry_meta_get(key: str) -> str | None:
    # 2026-09-20: same row_factory positional-index fix as
    # faa_registry_meta_get() above.
    with conn() as c:
        row = c.execute(
...
def opensky_registry_count() -> int:
    # 2026-09-20: same fix as faa_registry_count() above.
    with conn() as c:
        return c.execute("SELECT COUNT(*) AS n FROM opensky_aircraft_registry").fetchone()["n"]
...
def init_db_v26() -> None:
    ...
    2026-08-20: second, independent bug in that same fix, found while
    building db.init_db_all() and running the full migration chain against
    a genuinely empty database for the first time ever (every real
    deployment so far already had train_events from before this code
    existed, so this was never actually exercised). The CREATE-TABLE-first
    filter checked stmt.upper().startswith("CREATE TABLE") -- but the very
    first statement in SCHEMA_V26 begins with this docstring's own leading
    SQL comment (the "-- 2026-07-28: train_events..." block above), not
    the CREATE TABLE keyword, so .startswith() never matched it and
    train_events was silently never created by this function on a fresh
    DB. The later ALTER TABLE then failed outright with "no such table:
    train_events" instead of silently no-op'ing. Fixed by stripping
    leading comment lines before the startswith check."""
...
def init_db_v34() -> None:
    """Apply v34 schema -- flight_ooooi_times, a durable per-GUFI record of
    TFMS's real airline-reported OOOI times (airlineOutTime/OffTime/OnTime/
    InTime) plus originalDeparture/originalArrival (the actual scheduled
    times FIXM/TFMS carries in flightTimeData -- see tfms_parser.py's
    _handle_flight_times docstring). Added 2026-08-20 so a genuine
    scheduled-vs-actual on-time/delay computation is possible per flight
    number over time -- before this, TFMS's OOOI block only ever fired a
    transient watchlist alert (shared/watchlist.watchlist_event_hit) and was
    never persisted anywhere durable, so no historical on-time rate could
    ever be computed. See SCHEMA_V34 comment above."""
    with conn() as c:
        for stmt in SCHEMA_V34.strip().split(";"):
            stmt = stmt.strip()
            if not stmt:
                continue
            try:
...
def init_db_v35() -> None:
    """Apply v35 schema -- session_grants. Added 2026-08-20 as part of the
    dual-GPG-key signing model (see the operator/agent trust-boundary work
    the same day, docs/COMPLIANCE_SECURITY.md and CLAUDE.md's "Agent
    signing" section). A session_grant is a bounded, human-issued exemption
    from the per-signature approval-gate round-trip (see
    scripts/sudo-approval-gate.sh) for a specific command_pattern -- e.g.
    "sign-manifest:agent-key" -- so a human doing a long batch of agent
    work doesn't have to tap Allow on their phone for every single
    signature. Deliberately separate from the existing count_recent_
    approvals()/promotion_candidate mechanism (create_approval_request()'s
    docstring): that one is a slow, multi-day trust-building path toward a
    PERMANENT standing exemption; this is a short, explicit, always-bounded
    grant a human issues on purpose for one sitting. A grant changes
    friction, never the record -- every individual use of an active grant
    still gets its own db.audit() row (see scripts/agent-sign-manifest.sh)."""
    with conn() as c:
        for stmt in SCHEMA_V35.strip().split(";"):
            stmt = stmt.strip()
            if not stmt:
                continue
            try:
...
SCHEMA_V36 = """
ALTER TABLE nas_programs ADD COLUMN key_scheme INTEGER;
ALTER TABLE nas_programs ADD COLUMN legacy_correlate_id TEXT;
UPDATE nas_programs SET key_scheme = 1
    WHERE type IN ('GDP', 'GS') AND key_scheme IS NULL;
"""


def init_db_v36() -> None:
    """Apply v36 schema -- key_scheme + legacy_correlate_id on nas_programs.
    Added 2026-08-23 alongside the GDP/GS ElementTree-truthiness fix in
    tfms_parser.py (see _first_present() there -- same bug class as the
    FDPS squawk bug, tests/ingest/test_fdps_element_truthiness.py): every
    GDP/GS program_id written before this fix was silently keyed off
    advisoryValidPeriod's start instead of the intended
    cumulativeProgramPeriod (GDP) / groundStopPeriod (GS) field, because
    an `or` chain over childless leaf Elements always falls through to
    the last candidate. Fixing the parser changes what program_id a
    currently-active program computes to on its very next update --
    instead of upserting the existing row, it would silently create a
    duplicate under the new key and leave the old row orphaned (nothing
    in this codebase ever marks TFMS-sourced nas_programs rows inactive
    -- deactivate_absent_programs() is only called from the REST
    poller/fetchers/nas.py path, a completely different program_id
    scheme, confirmed via grep -rn "deactivate_absent_programs" src/).

    key_scheme distinguishes the two eras without deleting, renaming, or
    silently re-keying anything:
      NULL -- not applicable (REST-sourced nas.py rows, AFP rows -- AFP
              was never subject to this bug, its handler already used
              explicit `is not None` selection from day one)
      1    -- GDP/GS row written under the old, wrong key ("legacy"/"v1")
      2    -- GDP/GS row written under the fixed key ("v2"), stamped by
              write_tfms_programs() going forward

    The UPDATE above is a one-time backfill: every GDP/GS row that
    exists at the moment this migration first runs predates the fix by
    definition (the fix and this migration ship together), so it gets
    stamped key_scheme=1. Safe if this ever re-runs against a still-NULL
    GDP/GS row later too -- treating unknown as legacy is the
    conservative default, not a correctness risk.

    legacy_correlate_id is a best-effort link from a new (v2) row to the
    most recent (v1) row for the same type+facility (see
    find_legacy_nas_program()) within a 48h window -- lets a program
    that straddles the cutover still be recognized as the same
    real-world program across the key change without an exact-match
    join, satisfying the operator's ask for 30-day cross-cutover
    correlation.

    Retention: nothing in this repo currently deletes nas_programs rows
    at all (confirmed via grep -rn "nas_programs" src/ for
    DELETE/prune -- zero hits) -- key_scheme=1 ("v1"/legacy/pre-fix,
    e.g. for demo purposes) rows are not at any deletion risk today by
    default. This column exists to make that "kept for demo/correlation"
    property explicit and queryable rather than incidental. If a prune
    job is ever added for nas_programs, it MUST exempt key_scheme=1 rows
    younger than 30 days (operator's stated minimum 14, preferred 30)
    from their fetched_at, not just delete on some universal age cutoff."""
    with conn() as c:
        for stmt in SCHEMA_V36.strip().split(";"):
            stmt = stmt.strip()
            if not stmt:
                continue
            try:
...
SCHEMA_V37 = """
ALTER TABLE watchlist_entries ADD COLUMN departure_delay_min INTEGER;
"""


def init_db_v37() -> None:
    """Apply v37 schema -- departure_delay_min on watchlist_entries.

    Operator directive 2026-08-23: a transient flight watchlist entry's
    auto_remove_at (see _default_auto_remove_at() in
    web/routes/watchlist.py -- scheduled_arrival + 6h when known at
    add-time, else added_at + 24h fallback) should extend by the real
    departure delay the moment the flight actually goes airborne, not
    just sit on the original estimate -- a flight that pushed back 2h
    late still needs ~2h more runway on the back end before its now-later
    actual arrival. See shared.watchlist.extend_auto_remove_for_delay(),
    called from ingest/parsers/tfms_parser.py::_handle_flight_times() the
    moment a real airlineOffTime is observed.

    departure_delay_min is purely additive (never drops/renames anything)
    and serves double duty: it records the real delay applied (0 for an
    on-time-or-early departure, still worth recording so a repeat isn't
    mistaken for "never processed"), and its NULL-ness is the idempotency
    guard -- extend_watchlist_auto_remove_for_delay() only ever fires once
    per entry (`WHERE departure_delay_min IS NULL`), since TFMS can resend
    the same airlineOffTime on a later message and re-extending on every
    resend would compound indefinitely."""
    with conn() as c:
        for stmt in SCHEMA_V37.strip().split(";"):
            stmt = stmt.strip()
            if not stmt:
                continue
            try:
...
# 2026-09-04 (operator directive): stdds_rvr and metar_snapshot were both
# pure upsert-by-key tables with NO history counterpart at all -- unlike
# stdds_safety_status, which got stdds_safety_status_history back in v30
# for exactly this reason. Confirmed live: stdds_rvr held only 16 rows
# total (one per airport+runway, current snapshot), not because RVR
# reporting "just started," but because nothing ever wrote a history row
# -- upsert_stdds_rvr() only ever overwrote the current-state table.
# Needed for two things: (1) correlating visibility trend against the
# stdds_safety bitmask reverse-engineering effort (SCHEMA_V30's stated
# purpose), (2) using METAR wind direction over time to infer which
# runway configuration was active when, which needs actual history to
# correlate against -- a single current snapshot can't answer "what was
# the wind doing three days ago." wind_dir_deg is a NEW stored column
# (metar_snapshot never persisted it -- poller.fetchers.metar.
# parse_wind_dir() existed but was only ever called transiently by the
# pusher's wind-change detection, never written to any table).
SCHEMA_V44 = """
...
def init_db_v44() -> None:
    """Apply v44 schema -- stdds_rvr_history + metar_history (both were
    upsert-only with no history before this). See SCHEMA_V44 comment
    above for why."""
    with conn() as c:
        c.executescript(SCHEMA_V44)


def insert_metar_history(station: str, raw_metar: str, ceiling_ft: int | None,
                         visibility_sm: float | None, wind_kt: int | None,
                         wind_dir_deg: int | None, precip_code: str | None,
                         obs_time: float | None) -> None:
    """Append one METAR observation. Unlike upsert_metar() (current
    snapshot, overwritten every fetch), this is logged unconditionally on
    every successful fetch (every 300s per FETCH_SCHEDULE) -- METARs are
    themselves periodic observations, not a continuously-varying signal
    like RVR, so "log every fetch" is the natural granularity rather than
    change-detection."""
    with conn() as c:
        c.execute("""
            INSERT INTO metar_history
                (station, raw_metar, ceiling_ft, visibility_sm, wind_kt,
                 wind_dir_deg, precip_code, obs_time, recorded_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, unixepoch())
        """, (station, raw_metar, ceiling_ft, visibility_sm, wind_kt,
              wind_dir_deg, precip_code, obs_time))
...
def create_session_grant(command_pattern: str, granted_by: str,
    ...
    """Human-issued only -- see scripts/grant-agent-session.sh, the one
    intended caller. Nothing agent-side can call this to grant itself
    anything; it can only check for/consume an existing grant (see
    get_active_session_grant). Default TTL 4 hours -- deliberately bounded,
    never open-ended; pass a smaller ttl_seconds for a shorter task-scoped
    grant. scope is informational ("session" | "task" | anything else the
    caller wants to record) -- expiry is what's actually enforced, not the
    label."""
    import uuid
    ...
    with conn() as c:
        c.execute("""
            INSERT INTO session_grants
                (id, command_pattern, granted_by, reasoning, scope, granted_at, expires_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (grant_id, command_pattern, granted_by, reasoning, scope, now, expires_at))
    audit("session_grant_created", "admin", None, None, {
        "grant_id": grant_id, "command_pattern": command_pattern,
        "granted_by": granted_by, "reasoning": reasoning,
        "scope": scope, "ttl_seconds": ttl_seconds, "expires_at": expires_at,
    })
    return {"id": grant_id, "command_pattern": command_pattern,
            "expires_at": expires_at, "scope": scope}


def get_active_session_grant(command_pattern: str) -> dict | None:
    """Most recent non-revoked, non-expired grant for this exact pattern,
    or None. Callers (e.g. scripts/agent-sign-manifest.sh) MUST still
    db.audit() every individual use even when a grant is active -- this
    function only answers "is friction waived right now", it is not itself
    the audit record."""
    now = time.time()
    with conn() as c:
        row = c.execute("""
            SELECT * FROM session_grants
            WHERE command_pattern = ? AND expires_at > ? AND revoked_at IS NULL
            ORDER BY granted_at DESC LIMIT 1
        """, (command_pattern, now)).fetchone()
        return dict(row) if row else None


def revoke_session_grant(grant_id: str, revoked_by: str) -> bool:
    """Explicit early revoke -- a human can end a grant before its natural
    expiry. Returns False if the grant doesn't exist or was already
    revoked/expired (idempotent, not an error)."""
    now = time.time()
    with conn() as c:
        cur = c.execute(
            "UPDATE session_grants SET revoked_at = ? WHERE id = ? AND revoked_at IS NULL",
            (now, grant_id),
        )
        if cur.rowcount == 0:
            return False
    audit("session_grant_revoked", "admin", None, None,
          {"grant_id": grant_id, "revoked_by": revoked_by})
    return True


def upsert_flight_ooooi(gufi: str, callsign: str | None = None,
                         origin: str | None = None, destination: str | None = None,
                         airline_out_time: str | None = None,
                         airline_off_time: str | None = None,
                         airline_on_time: str | None = None,
                         airline_in_time: str | None = None,
                         original_departure: str | None = None,
                         original_arrival: str | None = None,
                         flight_status: str | None = None) -> None:
    """Upsert one flight's OOOI/scheduled-time record, keyed by GUFI.
    Only overwrites a column when the caller actually supplies a non-None
    value for it (COALESCE against the existing row) -- TFMS sends this
    data across several distinct message types over a flight's lifecycle
    (FlightModify/FlightTimes, departureInformation, arrivalInformation),
    each carrying a different subset of the OOOI/original-time fields, so
    a later message must not blank out an earlier one's data."""
    airline, flight_num = None, None
    if callsign:
        import re
        m = re.match(r"^([A-Za-z]+)(\d+[A-Za-z]?)$", callsign.strip())
        if m:
            airline, flight_num = m.group(1).upper(), m.group(2)
    with conn() as c:
        c.execute("""
            INSERT INTO flight_ooooi_times
...
def list_flight_numbers_with_ooooi_data(days: int = 14) -> list[tuple[str, str]]:
    """Distinct (airline, flight_num) pairs with at least one
    flight_ooooi_times row in the last `days` -- the "known universe" for
    get_flight_ontime_history(), e.g. for a periodic digest to iterate over
    without needing its own separate list of flight numbers to check."""
    cutoff = time.time() - days * 86400
    with conn() as c:
        rows = c.execute("""
            SELECT DISTINCT airline, flight_num FROM flight_ooooi_times
            WHERE airline IS NOT NULL AND flight_num IS NOT NULL AND updated_at >= ?
        """, (cutoff,)).fetchall()
    return [(r["airline"], r["flight_num"]) for r in rows]


def get_flight_ontime_history(airline: str, flight_num: str, days: int = 14,
                               drift_minutes_flag: int = 10) -> dict:
    """Real (not proxied) on-time departure/arrival history for a flight
    number, computed from actual airline-reported OOOI times vs. TFMS's
    originalDeparture/originalArrival -- both genuinely present in the same
    flight_ooooi_times row (see upsert_flight_ooooi). A record only counts
    toward departure stats if it has BOTH airline_out_time and
    original_departure (same for arrival/airline_in_time/original_arrival)
    -- partial records are excluded rather than guessed at.

    Also flags delay DRIFT (2026-08-20 follow-on) -- mean delay across the
    oldest third of samples vs. the newest third, same mechanism as
    analyze_train_patterns()'s schedule_drift_flags (mirrored deliberately
    for consistency, not reinvented), flagged when the shift exceeds
    drift_minutes_flag. This is a genuinely different question from the
    on-time rate above: a flight can hold a steady 90% on-time rate while
    still drifting (e.g. consistently 5 min late three weeks ago, now
    consistently 18 min late -- rate barely moves, trend is real). Same
    honest caveat as the train version: this is a real mechanism
    compounding a genuine baseline from 2026-08-20 forward, not a
    multi-week lookback on day one -- needs at least 3 samples split into
    thirds (i.e. practically, several days/weeks of the flight number
    actually being watchlisted) before a drift value is meaningful, so
    "drift": None with too few samples is expected, not a bug.

    Returns {"sample_days": int, "departure": {...} | None, "arrival": {...} | None}.
    Each present stat dict has count, on_time_count (<=15 min late),
    on_time_rate, avg_delay_minutes, records (list of {date,
    delay_minutes}), and drift: {shift_minutes, old_avg_delay_minutes,
    new_avg_delay_minutes, flagged} | None (None when fewer than 3 records).
    "insufficient_data": True is set at the top level when sample_days is 0
    -- this table only started being populated 2026-08-20, so a 0-sample
    result for any date range including days before that is expected, not
    a bug."""
    cutoff = time.time() - days * 86400
    with conn() as c:
        rows = c.execute("""
            SELECT gufi, airline_out_time, airline_in_time,
                   original_departure, original_arrival, updated_at
            FROM flight_ooooi_times
            WHERE airline = ? AND flight_num = ? AND updated_at >= ?
            ORDER BY updated_at ASC
        """, (airline.upper(), flight_num, cutoff)).fetchall()
    ...
    dep_records, arr_records = [], []
    seen_days = set()
    for r in rows:
        seen_days.add(datetime.fromtimestamp(r["updated_at"], tz=timezone.utc).strftime("%Y-%m-%d"))
        actual_out = _parse(r["airline_out_time"])
        sched_out = _parse(r["original_departure"])
        if actual_out is not None and sched_out is not None:
            dep_records.append({
                "date": datetime.fromtimestamp(sched_out, tz=timezone.utc).strftime("%Y-%m-%d"),
                "delay_minutes": round((actual_out - sched_out) / 60.0, 1),
            })
        actual_in = _parse(r["airline_in_time"])
        sched_in = _parse(r["original_arrival"])
        if actual_in is not None and sched_in is not None:
            arr_records.append({
                "date": datetime.fromtimestamp(sched_in, tz=timezone.utc).strftime("%Y-%m-%d"),
                "delay_minutes": round((actual_in - sched_in) / 60.0, 1),
            })

    def _drift(delays: list[float]) -> dict | None:
        # Mirrors analyze_train_patterns()'s schedule_drift_flags exactly:
        # oldest-third average vs newest-third average. `delays` is already
        # chronological (records built from `rows`, itself ORDER BY
        # updated_at ASC above) -- do not re-sort here.
        if len(delays) < 3:
            return None
        third = max(1, len(delays) // 3)
        old_avg = sum(delays[:third]) / third
        new_avg = sum(delays[-third:]) / third
        shift = new_avg - old_avg
        return {
            "shift_minutes": round(shift, 1),
            "old_avg_delay_minutes": round(old_avg, 1),
            "new_avg_delay_minutes": round(new_avg, 1),
            "flagged": abs(shift) >= drift_minutes_flag,
        }

    def _stats(records: list[dict]) -> dict | None:
        if not records:
            return None
        delays = [r["delay_minutes"] for r in records]
        on_time = sum(1 for d in delays if d <= 15.0)
        return {
            "count": len(records),
            "on_time_count": on_time,
            "on_time_rate": round(on_time / len(records), 3),
    ...
    return {
        "sample_days": len(seen_days),
        "insufficient_data": len(seen_days) == 0,
        "departure": _stats(dep_records),
        "arrival": _stats(arr_records),
    }
...
def upsert_safety_status(airport: str, control: str | None,
    ...
    with conn() as c:
        ...
        previous = row["status_bitmask"] if row else None
        c.execute("""
...
# ============================================================================
# CIFP (Coded Instrument Flight Procedures, ARINC 424) -- DC-area SID/STAR/
# approach legs, holds, and a fix catalog for resolving filed-route waypoint
# idents to real coordinates. Built 2026-09-05 from a real, byte-verified
# parse of CIFP cycle 2609 (CIFP_260903.zip, the same file
# src/poller/skills/faa_cifp_pull.py already fetches automatically). See
# src/poller/skills/faa_cifp_parse.py (the parser that populates these
# tables on each new cycle) and src/common/cifp_lookup.py (the read-only
# query API other modules should use -- never query these tables directly).
# ============================================================================
SCHEMA_V45 = """
...
def init_db_v45() -> None:
    """Apply v45 schema -- CIFP fixes/procedure-legs/holds + meta."""
    with conn() as c:
        c.executescript(SCHEMA_V45)


# ── TBFM eta arc-type -- SCHEMA_V46, 2026-09-05 ─────────────────────────────
# tbfm_parser._parse_tbfm_air() sets a sequence's `eta` from the first
# available of eta_rwy -> eta_mfx -> eta_dfx -> eta_sfx, but never recorded
# WHICH arc it used. Those are ETAs to genuinely different points:
#
#   eta_rwy  runway threshold  -- already the answer; nothing to add
#   eta_mfx  the meter fix     -- the only one our CIFP fix geometry matches
#   eta_dfx  a descent fix     -- a different point entirely
#   eta_sfx  a speed fix       -- likewise
#
# fdps_parser's arrival_time enrichment treated the stored value as a
# meter-fix ETA unconditionally and added CIFP fix->runway transit time on
# top. When the message actually carried eta_rwy that transit got counted
# TWICE and the estimate ran long; for dfx/sfx the transit was measured
# from the wrong point altogether. Recording the arc makes the enrichment
# able to tell the cases apart instead of guessing.
#
# ALTER TABLE (not a table rebuild) with a duplicate-column guard, so this
# is idempotent and safe to re-run -- same pattern as the other vN
# migrations here. Pre-existing rows keep eta_kind NULL, which callers must
# treat as UNKNOWN (never as "mfx"); TBFM re-upserts every row continuously
# and the consumers all apply a 1800s freshness gate, so the NULL backlog
# ages out on its own within ~30 minutes of deploy.
def init_db_v46() -> None:
    """Apply v46 schema -- tbfm_sequences.eta_kind (which ETA arc TBFM
    actually supplied). See the comment block above."""
    with conn() as c:
        cols = {r[1] for r in c.execute("PRAGMA table_info(tbfm_sequences)")}
        if "eta_kind" not in cols:
...
def cifp_meta_set(key: str, value: str) -> None:
    with conn() as c:
        c.execute(
            "INSERT OR REPLACE INTO cifp_meta (key, value) VALUES (?, ?)",
            (key, value),
        )


def cifp_meta_get(key: str) -> str | None:
    with conn() as c:
        row = c.execute("SELECT value FROM cifp_meta WHERE key=?", (key,)).fetchone()
    return row["value"] if row else None


def cifp_replace_all(cycle: str, fixes: list[dict], legs: list[dict],
                      holds: list[dict]) -> dict:
    """Wholesale-replace all three CIFP tables for a new cycle. Refuses an
    empty replace (same lesson as faa_upsert_ladd's C-31 fix) -- a fetch/
    parse failure must never silently wipe the existing, still-valid
    prior-cycle data down to zero."""
    if not fixes or not legs:
        log.error(
            "cifp_replace_all: refusing to replace CIFP data with an "
            "empty result (fixes=%d legs=%d holds=%d) -- leaving the "
            "existing cycle %s in place. A parse producing nothing "
            "usable means the fetch/parse failed, not that the data "
            "should be cleared.",
            len(fixes), len(legs), len(holds), cifp_meta_get("cycle"),
        )
        return {"ok": False, "reason": "empty_result"}

    with conn() as c:
        c.execute("DELETE FROM cifp_fixes")
        c.executemany(
            """INSERT OR REPLACE INTO cifp_fixes
               (ident, icao_region, type, lat, lon, name, parent_airport, cycle)
               VALUES (:ident, :icao_region, :type, :lat, :lon, :name,
                       :parent_airport, :cycle)""",
            fixes,
        )
        c.execute("DELETE FROM cifp_procedure_legs")
        c.executemany(
            """INSERT INTO cifp_procedure_legs
               (airport, proc_type, procedure, transition, seq, fix,
                fix_region, fix_type, path_term, turn, lat, lon, alt_desc,
                alt1, alt2, speed_limit, cycle)
               VALUES (:airport, :proc_type, :procedure, :transition, :seq,
                       :fix, :fix_region, :fix_type, :path_term, :turn,
                       :lat, :lon, :alt_desc, :alt1, :alt2, :speed_limit,
                       :cycle)""",
            legs,
        )
        c.execute("DELETE FROM cifp_holds")
        if holds:
            c.executemany(
                """INSERT INTO cifp_holds
                   (airport, proc_type, procedure, transition, seq, fix,
                    fix_type, lat, lon, hold_type, hold_meaning,
                    turn_direction, inbound_course_mag, leg_time_min,
                    leg_dist_nm, alt_desc, alt1, alt2, speed_limit, cycle)
                   VALUES (:airport, :proc_type, :procedure, :transition,
                           :seq, :fix, :fix_type, :lat, :lon, :hold_type,
                           :hold_meaning, :turn_direction,
                           :inbound_course_mag, :leg_time_min, :leg_dist_nm,
                           :alt_desc, :alt1, :alt2, :speed_limit, :cycle)""",
                holds,
            )

    cifp_meta_set("cycle", cycle)
    cifp_meta_set("last_full_import", time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()))
    return {"ok": True, "cycle": cycle, "fixes": len(fixes), "legs": len(legs),
            "holds": len(holds)}
