...
def _index(conn, rel_path: str, title: str, content: str, tags: str, ingest_method: str) -> None:
...
def _write_review_note(category: str, key: str, entry: dict, trigger: str | None, first_mover_reasons: list[str]) -> None:
    ...
    # 2026-08-16 drift audit: a `with` block (get_conn() is a context
    # manager) can't leak the connection on a raise inside _index(),
    # same guarantee the original try/finally was written for.
...
def _write_entity_note(category: str, key: str, entry: dict, trigger: str) -> None:
    ...
    webdav_client.put(rel_path, note)
    # 2026-08-16 drift audit: see the same fix on the novel-finding writer
    # above -- get_conn()'s `with` block can't leak on an _index() raise.
    with get_conn() as conn:
