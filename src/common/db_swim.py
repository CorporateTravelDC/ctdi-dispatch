...
def upsert_stdds_rvr(airport: str, runway: str,
    ...
    2026-09-04: also appends to stdds_rvr_history (see common/db.py's
    SCHEMA_V44) whenever any comparable field actually changed --
    upsert_stdds_rvr() was previously pure current-state with no history
    at all, unlike stdds_safety_status's parallel history table. One
    read + one/two writes, same conn()/transaction -- no extra round
    trip."""
    with conn() as c:
        prev = c.execute(
            "SELECT touchdown_rvr_ft, touchdown_trend, midpoint_rvr_ft, "
            "midpoint_trend, rollout_rvr_ft, rollout_trend, "
            "edge_light_setting, centerline_light_setting "
            "FROM stdds_rvr WHERE airport=? AND runway=?", (airport, runway)
        ).fetchone()
        new_vals = (touchdown_rvr_ft, touchdown_trend, midpoint_rvr_ft,
                    midpoint_trend, rollout_rvr_ft, rollout_trend,
                    edge_light_setting, centerline_light_setting)
        changed = prev is None or tuple(prev) != new_vals
        ...
        if changed:
            c.execute("""
                INSERT INTO stdds_rvr_history
                    (airport, runway, touchdown_rvr_ft, touchdown_trend,
                     midpoint_rvr_ft, midpoint_trend, rollout_rvr_ft, rollout_trend,
                     edge_light_setting, centerline_light_setting, recorded_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (airport, runway, *new_vals, last_seen))
