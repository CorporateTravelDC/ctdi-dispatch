...
new always-on background thread):
  ...
  - All state is in-process (module-level dicts) except the silence
    ...
    to a fresh baseline on restart. CORRECTED 2026-09-20 -- this docstring
    previously claimed that reset "is fine... a 15-min window mostly
    self-heals within one window anyway"; confirmed wrong for the FIRST
    window: right after any restart, prior_count=0 trivially satisfies the
    multiplier check, so window_count >= floor alone decides "escalating"
    for the first _WINDOW_SECS -- a false-positive burst, not routine
    self-healing. Root-caused after the operator reported every alert
    reading as an escalation; see the cold-start guard in record_event()/
    get_sector_summary() below for the fix.
...
# Cold-start guard (2026-09-20, operator-confirmed root cause): captured once
# at import. record_event()/get_sector_summary() both refuse to report
# escalating=True until 2*_WINDOW_SECS have elapsed since this process
# started -- before that, prior_count is artificially low/zero (this
# process hasn't been alive long enough to have a real prior window), which
# trivially satisfies the multiplier check and collapses "escalating" to
# just window_count >= floor. A single timestamp comparison, not a new
# persistence mechanism -- record_event() runs on every ingest message
# (thousands/hour), so file I/O here would reintroduce the exact
# lock-contention problem the per-topic throttle fix above deliberately
# avoided for this same reason (see the 2026-09-04 comment near
# _THROTTLE_STATE_FILE). window_count/prior_count/sector_silenced/
# feed_silenced are unaffected by this guard -- only the escalating
# boolean itself is gated.
...
def record_event(sector: str, feed_name: str, isolate: bool = False) -> dict:
    ...
    with _lock:
        ...
        # Cold-start guard -- see _PROCESS_START above for why. Only this
        # boolean is gated; window_count/prior_count/silenced flags below
        # still report their real values either way.
        escalating = (
            now - _PROCESS_START >= 2 * _WINDOW_SECS
            and window_count >= floor
...
def get_sector_summary() -> dict:
    ...
    with _lock:
        ...
        for sector in sorted(all_sectors):
            ...
            # Same cold-start guard as record_event() -- keep these two
            # formulas identical (see _PROCESS_START above for why).
            escalating = (
                now - _PROCESS_START >= 2 * _WINDOW_SECS
                and window_count >= _ESCALATE_FLOOR
