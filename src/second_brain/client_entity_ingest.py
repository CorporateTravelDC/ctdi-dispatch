...
def init_entities_table(conn) -> None:
    """No-op on Postgres -- entities already exists (pg_schema/0055), same
    pattern as index_db.init_db(). Kept callable so main() below doesn't
    need its own backend-aware branch."""
    return None
...
def ingest_vcf(conn, path: str) -> dict:
...
def entity_summary(conn) -> dict:
    total = conn.execute("SELECT COUNT(*) AS n FROM entities").fetchone()["n"]
    by_category = {
        r["category"]: r["n"] for r in conn.execute(
            "SELECT category, COUNT(*) AS n FROM entities GROUP BY category ORDER BY 2 DESC"
        ).fetchall()
    }
