...
# 2026-09-04 (operator directive): empirically verified from 40,477 real
# stdds_safety_status_history transitions (2026-08-03 -- 2026-09-04,
# KDCA/KIAD/KBWI). For each group below, every listed bit position was
# checked pairwise across the ENTIRE history: they flip together on
# EVERY transition (never independently -- zero single-bit exceptions),
# and the pair's 2-bit value is only ever 00 or 11, never 01/10. That
# rules out a genuine 2-bit state enum (which would show all four
...
# historical RVR/METAR log existed to correlate against until this same
# session added stdds_rvr_history/metar_history (see common/db.py
# SCHEMA_V44). Correlating those against this table, once enough history
...
_STDDS_SAFETY_BIT_TO_GROUP: dict[str, dict[int, int]] = {
    airport: {bit: group[0] for group in groups for bit in group}
    for airport, groups in _STDDS_SAFETY_BIT_PAIRS.items()
}


def _collapse_safety_bits(bitmask_hex: str, airport: str) -> frozenset[int]:
    """Map a raw hex SafetyLogicHoldBar bitmask to its set of LOGICAL
    signal IDs for `airport`, using the verified pairing groups above --
    each group collapses to one representative ID (its lowest bit
    position); a bit with no known pairing keeps its own position as its
    ID. Airports with no entry in _STDDS_SAFETY_BIT_PAIRS (i.e. every
    zone outside KDCA/KIAD/KBWI) pass through with no collapsing."""
    bit_to_group = _STDDS_SAFETY_BIT_TO_GROUP.get(airport, {})
    n = int(bitmask_hex, 16)
    raw_bits = (i for i in range(n.bit_length()) if n & (1 << i))
    return frozenset(bit_to_group.get(b, b) for b in raw_bits)
...
def check_incursion_alert(record: dict, previous_bitmask: str | None) -> None:
    ...
    # 2026-09-04: report the COLLAPSED logical-signal count/set alongside
    # the raw bitmask -- see _collapse_safety_bits()'s docstring. Purely
    # informational (does not change whether this fires -- still any raw
    # bitmask change, per this function's own docstring); makes the
    # detail text readable instead of a 40+ hex-digit blob when most of
    # that width is verified-redundant paired bits.
    new_logical = _collapse_safety_bits(new_bitmask, airport)
    if previous_bitmask is None:
        detail = (f"{airport}: safety-logic status first seen -- raw bitmask "
                  f"{new_bitmask} ({len(new_logical)} logical signals set: "
                  f"{sorted(new_logical)})")
    else:
        prev_logical = _collapse_safety_bits(previous_bitmask, airport)
        newly_set = sorted(new_logical - prev_logical)
        newly_cleared = sorted(prev_logical - new_logical)
        detail = (f"{airport}: safety-logic status CHANGED -- raw bitmask "
                  f"{new_bitmask} (was {previous_bitmask}); logical signals "
                  f"set: +{newly_set} -{newly_cleared} "
                  f"(now {len(new_logical)} active, collapsed from "
                  f"{bin(int(new_bitmask, 16)).count('1')} raw bits)")
