...
#
# RE-CONFIRMED 2026-09-05 against a full, independently-written, byte-
# verified parse of CIFP cycle 2609 (CIFP_260903.zip -- see
# src/poller/skills/faa_cifp_parse.py, common/cifp_lookup.py, and
# docs/CIFP_DATA.md), scoped to all 21 DC/Baltimore-area airports' actual
# SID/STAR/approach legs (5,060 legs, 242 procedures), not just the
# general national FIX file used in the 2026-09-02 check above. Same
# result: PALEO/RAVNN/SWANN/WOOLY/FLUKY resolve to byte-identical
# coordinates via CIFP's own ENROUTE_WPT records; LUCIT/JIMBO/WAVER/MERIT
# exist somewhere nationally (regions K4/K5/K6, not DC-local) but are
# referenced by ZERO legs of any DC-area procedure in this cycle; SFARA
# has no record anywhere. This is now the second independent
# confirmation of the same conclusion -- if a future CIFP cycle ever
# does resolve one of the 5, common.cifp_lookup.resolve_fix() would be
# the place to check, not another one-off lookup.
...
# ── scope-sizing instrumentation (2026-09-05, operator question) ────────────
# "What would widening beyond DCA/IAD/BWI cost?" -- top-30 US airports, the
# 30 WFOs already watched, or all US international + major CAN/MEX pairs.
# That is unanswerable by guesswork: the multiplier depends entirely on
# which airports, and the busy ones (ATL/ORD/DFW/DEN/LAX) each carry far
# more metering traffic than the DC three.
#
# So: count what we currently DISCARD, by arrival airport, and log a
# periodic histogram. Counting only -- nothing is parsed further, nothing is
# stored, no DB write, no new rows. Cost is one dict increment per dropped
# record. Lets any candidate airport list be priced from real traffic before
# committing to it.
#
# Sizing context this is meant to inform (all measured 2026-09-05): the DB is
# already 23 GB and shared single-writer, the box has ~4.8 GB RAM free and
# tripped thermal LOCKDOWN five times today, and TBFM alone already logs
# "database is locked" failures AFTER a full 10s busy-wait -- 13 in one hour
# across five ingest units at the CURRENT 3-airport volume. Write contention,
# not disk or CPU, is the binding constraint.
_dropped_apt_counts: dict[str, int] = {}
_dropped_apt_last_log = 0.0
_DROPPED_APT_LOG_INTERVAL_S = 900  # 15 min


def _count_dropped_apt(apt: str | None) -> None:
    """Tally one filtered-out arrival airport; log a histogram every 15 min."""
    global _dropped_apt_last_log
    key = apt or "(none)"
    _dropped_apt_counts[key] = _dropped_apt_counts.get(key, 0) + 1
    now = _time.time()
    if _dropped_apt_last_log == 0.0:
        _dropped_apt_last_log = now
        return
    if now - _dropped_apt_last_log < _DROPPED_APT_LOG_INTERVAL_S:
        return
    _dropped_apt_last_log = now
    total = sum(_dropped_apt_counts.values())
    top = sorted(_dropped_apt_counts.items(), key=lambda kv: -kv[1])[:30]
    log.info("tbfm: scope-sizing -- dropped %d non-DC metering records in the "
             "last %ds across %d airports; top 30: %s",
             total, _DROPPED_APT_LOG_INTERVAL_S, len(_dropped_apt_counts),
             ", ".join(f"{a}={n}" for a, n in top))
    _dropped_apt_counts.clear()
...
def _parse_air_element(air: ET.Element, facility: str | None) -> dict | None:
    ...
    """
    # NOTE: the `apt not in DC_AREA_APTS` gate below is OURS, not the FAA's.
    # The subscription demonstrably carries national metering (live captures
    # show apt=PHX, apt=OAK), and everything outside DCA/IAD/BWI is dropped
    # here, before storage. Deliberate -- this is a DC dispatch platform --
    # but it means widening scope is a filter change, NOT a feed change.
    # _count_dropped_apt() measures what that would cost; see its docstring.
    apt = (air.get("apt") or "").upper() or None
    if apt not in DC_AREA_APTS:
        _count_dropped_apt(apt)
        return None
    ...
    #
    # 2026-09-05: also record WHICH arc won. These are ETAs to genuinely
    # different points (runway threshold / the meter fix / a descent fix /
    # a speed fix) and consumers cannot treat them alike -- fdps_parser's
    # arrival_time enrichment adds CIFP fix->runway transit time, which is
    # correct ONLY for "mfx", double-counts for "rwy" (already a runway
    # ETA), and is measured from the wrong point for "dfx"/"sfx". Storing
    # the arc lets it tell them apart instead of assuming. See
    # db.init_db_v46().
    eta_raw = None
    eta_kind = None
    for _field, _kind in (("eta_rwy", "rwy"), ("eta_mfx", "mfx"),
                          ("eta_dfx", "dfx"), ("eta_sfx", "sfx")):
        _val = _child_text(eta_elem, _field)
        if _val:
            eta_raw, eta_kind = _val, _kind
            break
...
def _check_tbfm_watchlist_hits(sequences: list[dict]) -> None:
    ...
    for s in sequences:
        ...
        # Append a runway-arrival estimate when one can be defended.
        #
        # 2026-09-05, CORRECTED: this was the THIRD consumer still keyed on
        # the hardcoded DC_METER_FIXES dict, which matches almost nothing
        # the live feed publishes (real meter fixes are HVQ/MULRR/CAPKO/
        # IGGGY/BILIT/CAVLR/KILMR/... -- see fdps_parser's note), so the
        # estimate was essentially never appended. It also treated the ETA
        # as a meter-fix ETA unconditionally, double-counting the
        # fix->runway leg whenever TBFM had actually supplied a runway ETA
        # (live distribution: rwy 18 / mfx 17, so about half the time).
        #
        # Now goes through cifp_lookup.runway_eta_epoch() -- the single
        # arc-aware implementation shared with fdps_parser.write_flight_event()
        # and poller/skills/tbfm_arrival_enrichment.py. Three consumers, one
        # implementation; keeping separate copies is what let them drift.
        if apt in DC_AREA_APTS:
            try:
                from common import cifp_lookup
                epoch = cifp_lookup.runway_eta_epoch(s.get("flight_id"), "K" + apt)
                if epoch:
                    est_iso = datetime.fromtimestamp(
                        epoch, tz=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
                    status += f", est. runway arrival {est_iso}"
            except Exception as e:
                log.debug("tbfm: runway-eta estimate failed for %s: %s",
                          s.get("flight_id"), e)
