...
def write_flight_event(parsed: dict) -> bool:
    ...
    # 2026-09-05: arrival_time enrichment. Confirmed live (flight_resolver.py's
    # own module comment) that this column has been hardcoded None on every
    # write since 2026-07-20 (0 of 579k+ rows ever populated) -- FDPS carries
    # no native arrival-time estimate, and resolve_arrivals()'s SWIM tier
    # filters on `arrival_time BETWEEN now AND cutoff`, so it structurally
    # never matched a DCA/IAD row despite SWIM data being fully healthy.
    # Fills it in for DC-area arrivals with an ACTIVE TBFM meter-fix sequence
    # (tbfm_sequences, joined on callsign -- TBFM never carries a GUFI) by
    # combining that sequence's own meter-fix ETA with CIFP's real runway-
    # threshold coordinates (common.cifp_lookup.estimate_runway_eta) -- a
    # static-geometry ESTIMATE, not a live schedule. None (not a guess) when
    # no TBFM sequence exists, the meter fix can't be resolved to real
    # coordinates, or the destination isn't DCA/IAD/BWI (TBFM's only scope).
    # Non-fatal.
    #
    # 2026-09-05, SECOND PASS -- the first version of this shipped broken and
    # produced 0 populated rows out of the 300 most recent flight_events.
    # It looked up the meter fix in tbfm_parser.DC_METER_FIXES, a hardcoded
    # 10-name dict, but that dict matches NOTHING this feed actually
    # publishes: the live tbfm_sequences meter_fix values (41,680 rows,
    # continuously fresh) are AIR/CAPKO/PLDGE/HVQ/MULRR/IGGGY/BUCKO/BILIT/
    # SITTR/WEEDU/BAL/CAPSS/CAVLR/KILMR/TRISH and friends -- not one of
    # PALEO/RAVNN/SWANN/WOOLY/FLUKY. DC_METER_FIXES had been validated
    # against CIFP (do these names exist?) but never against what TBFM
    # emits, so the `.get()` always missed and the enrichment silently
    # no-opped on every single flight.
    #
    # Resolve against the real cifp_fixes table instead -- which is exactly
    # what that dataset was imported for. Every one of the live fixes above
    # resolves there UNAMBIGUOUSLY (verified: one row each, region K6 except
    # AIR at K5), so plain resolve_fix() is safe without a region hint; it
    # returns None rather than guessing on a genuinely ambiguous ident.
    # The remaining live values (ZDC, DC_MET, IAD_MP, HVQ_BKW, J48_J75,
    # ZID-E, EAST, ...) are facility/meter-point pseudo-labels with no real
    # coordinates and correctly resolve to None. DC_METER_FIXES is kept as a
    # fallback only, for the handful of names it does cover.
    # Second half of the same bug: a flight carries MANY concurrent TBFM
    # sequence rows (UAL1635 had 42), and db.get_tbfm_sequence_for_flight()
    # returns only the FRESHEST. TBFM interleaves real fixes with
    # facility/meter-point pseudo-labels, so the freshest row is usually a
    # pseudo-label (UAL1635's was 'ZDC') -- which resolves to nothing, so
    # even with CIFP resolution wired in the enrichment still produced 0
    # rows. Walk all fresh sequences newest-first and take the first whose
    # meter fix actually resolves to real coordinates.
    arrival_time = None
    try:
        dest_bare = _norm_airport(parsed.get("destination"))
        if dest_bare and dest_bare.upper() in ("DCA", "IAD", "BWI") and callsign:
            from common import cifp_lookup
            arrival_time = cifp_lookup.runway_eta_epoch(
                callsign, "K" + dest_bare.upper())
    except Exception as e:
        log.debug("fdps: arrival_time enrichment failed for %s (non-fatal): %s",
                  callsign or flight_id, e)
