...
# processed even though it's older than the flat floor). Everything
...
#
# 2026-09-18: floor raised 2h -> 8h (operator directive, live-tested during
# the Postgres cutover's real ~7-8h SWIM outage). Standing SLA target as of
# tonight: a full working day's outage should fully recover -- no truncation
# -- on reconnect, not just the newest 10% of it. Beyond 8h, the
# BACKLOG_RECENT_FRACTION safety net still applies unchanged.
BACKLOG_STALE_SECONDS = int(os.getenv("SWIM_BACKLOG_STALE_SECONDS", "28800"))       # 8h
...
async def run(cfg: NmsConfig, stop: asyncio.Event) -> None:
    ...
    for feed_name, (feed_cfg, handler) in _FEED_HANDLERS.items():
        if feed_name.lower() in _skip_feeds:
            # BUG FIXED 2026-08-23: this used to call
            # _db.upsert_feed_skip(f"push:{feed_name}", ..., "disabled: ...")
            # here -- correct back when one container ran every SWIM feed and
            # a skip meant "genuinely disabled platform-wide". In the current
            # per-feed-container split, EVERY container's SWIM_NMS_SKIP_FEEDS
            # correctly lists the five feeds it does NOT own -- skipping is
            # the normal, expected case for 5 of 6 feeds in every container,
            # not a real "this feed is off" signal. feed_state is a single
            # table shared across all 7 containers, keyed only by feed name
            # with no notion of which container is authoritative for it, so
            # this write silently clobbered the real owning container's
            # genuine health status the moment any sibling container
            # (re)started -- confirmed live: push:itws showed "disabled:
            # SWIM_NMS_SKIP_FEEDS" while the itws container was actively
            # connected and processing real severe-weather alerts. Log only;
            # never write shared state for a feed this container doesn't own.
            log.info("swim_client %s: skipped via SWIM_NMS_SKIP_FEEDS (not this container's feed)",
                      feed_name)
            continue
