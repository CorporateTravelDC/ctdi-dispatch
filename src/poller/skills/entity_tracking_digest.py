...
def _fetch_new_findings(since_iso: str) -> list[dict]:
    with get_conn() as conn:
        ...
        # 2026-09-19: dict_row (Postgres) supports only key access, unlike
        # sqlite3.Row which supported both -- see db_backend.py's module
        # docstring for the wider incident this pattern caused tonight.
        return [
            {"path": r["path"], "filename": r["filename"], "category": r["category"],
             "tags": r["tags"], "ingest_method": r["ingest_method"], "indexed_at": r["indexed_at"]}
            for r in rows
        ]
