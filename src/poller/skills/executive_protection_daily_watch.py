...
common.entity_tracking from creation.
...
OLLAMA_MODEL = "corporatetraveldc-pi5-executive-protection-daily-watch:latest"  # dedicated Phase-4 model, persona + skill layer in its Modelfile SYSTEM
...
_SPLIT_TASK_PREAMBLE = """You are writing the daily executive-protection/
...
-- and a FRAMING INSTRUCTION at the end telling you which single
...
_FRAMING_INSTRUCTIONS = {
    "ops": (
        ...
        "operational relevance -- counter-UAS/physical threat "
        "developments, cybersecurity/targeted-attack trends aimed at "
        "service providers like this business."
    ),
    "ep": (
        "FRAMING INSTRUCTION: write TODAY'S DEVELOPMENTS focused on "
        "professional development -- new or newly-available "
        "training/certification opportunities (trauma/critical care, "
        "security-driving, close protection) worth knowing about."
    ),
}
...
def _generate_framing(flavor: str, base_prompt: str, headline_block: str) -> tuple[str, bool]:
    ...
    result = llm_generate(
        ...
        # Kept this skill's own originally-measured 1560s timeout (see the
        # old shared-call comment this replaced) rather than reusing
        # another sibling's value -- each call now does less work, so the
        # original budget is slack, not tight.
        timeout=1560, allow_anthropic=False,
    )
    if result:
