...
def _table_last_write_age(conn, table: str, ts_col: str, ts_type: str) -> float | None:
    # Explicit AS alias + key access (not row[0]) -- keeps this correct
    # under both the sqlite3.Row and the postgres dict_row backends (see
    # docs/POSTGRES_MIGRATION.md §4 / common.db_backend's row[0]-positional
    # access audit note).
    if ts_type == "epoch":
        row = conn.execute(f"SELECT MAX({ts_col}) AS max_ts FROM {table}").fetchone()
        max_ts = row["max_ts"]
        if max_ts is None:
            return None
...
def _critical_field_null_rate(conn, table: str, ts_col: str, ts_type: str,
    ...
    if ts_type == "epoch":
        ...
        total = conn.execute(
            f"SELECT COUNT(*) AS n FROM {table} WHERE {ts_col} > ?", (cutoff,)
        ).fetchone()["n"]
        if total == 0:
            return None
        null_count = conn.execute(
            f"SELECT COUNT(*) AS n FROM {table} WHERE {ts_col} > ? AND ({field} IS NULL OR {field} = 0)",
            (cutoff,),
        ).fetchone()["n"]
...
def run_check() -> list[str]:
    ...
    # Routed through common.db.conn() (Postgres migration Phase 1, see
    # docs/POSTGRES_MIGRATION.md §4) instead of its own raw sqlite3.connect()
    # -- follows DISPATCH_DB_BACKEND like every other reader of these tables.
