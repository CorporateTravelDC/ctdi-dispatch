...
def capture() -> dict:
    ...
    # docs/POSTGRES_MIGRATION.md §4) instead of its own raw, dynamically-
    # imported sqlite3.connect() calls -- follows DISPATCH_DB_BACKEND like
    # every other notams reader. Explicit AS aliases + key access (not
    # row[0]) keep this correct under both the sqlite3.Row and the
    # postgres dict_row backends.
    with db.conn() as conn:
        total = conn.execute("SELECT COUNT(*) AS n FROM notams").fetchone()["n"]
        last_hour = conn.execute(
            "SELECT COUNT(*) AS n FROM notams WHERE inserted_at > unixepoch()-3600"
        ).fetchone()["n"]
        classifications = [
        ...
        latest = conn.execute("SELECT MAX(inserted_at) AS n FROM notams").fetchone()["n"]
