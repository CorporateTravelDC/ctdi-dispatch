...
_split_framings() (see 2026-09-02 note below).
...
OLLAMA_MODEL = "corporatetraveldc-pi5-concierge-travel-daily-watch:latest"  # dedicated Phase-4 model, persona + skill layer in its Modelfile SYSTEM
...
_SPLIT_TASK_PREAMBLE = """You are writing the daily concierge/luxury-travel
...
luxury/concierge-travel headlines and a FRAMING INSTRUCTION at the end
telling you which single analytical framing to write.
...
_FRAMING_INSTRUCTIONS = {
    "ops": (
        ...
        "business-development relevance -- new destinations, off-market "
        "listings, exclusive partnerships/openings that could inform "
        "client offerings or referral relationships."
    ),
    "ep": (
        "FRAMING INSTRUCTION: write TODAY'S DEVELOPMENTS focused on the "
        "executive-protection/privacy angle -- anything relevant to "
        "secure/private travel arrangements, access-control at "
        "exclusive venues, or discretion-sensitive client movements."
    ),
}
...
def _generate_framing(flavor: str, base_prompt: str, headline_block: str) -> tuple[str, bool]:
    ...
    result = llm_generate(
        ...
        timeout=1470, allow_anthropic=False,
    )
    if result:
