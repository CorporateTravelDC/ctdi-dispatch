...
from common import cifp_lookup, db
...
def run_enrichment() -> dict:
    ...
    # -- follows DISPATCH_DB_BACKEND like every other flight_events/
    # tbfm_sequences reader. db.conn()'s sqlite path always sets
    # row_factory=sqlite3.Row (dict-like AND positionally iterable), but
    # postgres dict_row rows are NOT positionally iterable, so the below
    # switched from tuple-unpacking to explicit key access to stay correct
    ...
    for r in rows:
        flight_id, callsign, destination = r["flight_id"], r["callsign"], r["destination"]
