...
from ingest import failover as _failover
...
async def startup() -> None:
    ...
    # init_db_vN() calls are real SQLite DDL (some with SQLite-only
    # syntax translate_sql() doesn't cover), not a harmless idempotent
    # no-op on this backend -- confirmed live: one left a Postgres
    # transaction aborted and crash-looped every core container on
    # startup after the 2026-09-18 cutover.
    if db_backend.backend() != "postgres":
        ...
        db.init_db_v43()
        # 2026-09-04: stdds_rvr_history/metar_history -- web doesn't write
        # either, but a future admin/analysis endpoint reading them needs the
        # tables to exist even on a fresh DB where ingest/poller haven't run
        # yet.
        db.init_db_v44()
        # 2026-09-05: cifp_fixes/cifp_procedure_legs/cifp_holds -- web doesn't
...
_BOARD_REDACT_PATTERNS = [
    # WebDAV/vault hostname -- an internal infra hostname has no business
    # being handed to an anonymous, unauthenticated Tier-0 caller even
    # though it isn't a secret by itself (it's the address of a system
    # a determined attacker would target next).
    (re.compile(r"cloud\.csexecutiveservices\.com", re.IGNORECASE), "[internal-vault-host]"),
    # "app password"/"app-password"/APP_PASSWORD-style references -- not
    # the credential value itself (never posted to the board), but naming
    # which credential mechanism is in play is still reconnaissance value
    # an anonymous reader shouldn't get for free.
    (re.compile(r"app[\s_-]?password", re.IGNORECASE), "[credential-mechanism-redacted]"),
]


def _redact_board_body(text: str) -> str:
    """Mask known internal-infra/credential-mechanism references before a
    board message reaches an anonymous Tier-0 reader. Added 2026-08-24
    (ground-up audit + remediation pass): board reads are deliberately
    anonymous by design (Cowork's sandbox has no tailnet access, so
    token-gating reads would break the cross-environment channel
    entirely -- see the board's own design note in the vault, mirrored in
    this file's routing docstring) and that design choice is NOT being
    reversed here. What's fixed is narrower: real coordination traffic on
    this board has, in practice, included the vault's WebDAV hostname and
    references to the app-password mechanism protecting it -- neither is
    the credential itself, but both are reconnaissance value that doesn't
    need to be free. This is deliberately a narrow, pattern-based mask
    (same style as scrub-public-tree.py's SUBSTITUTIONS), not a general
    content-security filter -- it does not address whether OTHER
    sensitive material (e.g. personal-project excerpts) belongs on an
    anonymously-readable board at all; that's a content-policy question
    for whoever posts to the board, not something this function can fix
    by pattern-matching, and is intentionally left open here rather than
    silently "solved" by over-broad redaction of real coordination
    content."""
    for pattern, replacement in _BOARD_REDACT_PATTERNS:
...
async def board_get(
    ...
    for m in msgs:
        if isinstance(m.get("body"), str):
            m["body"] = _redact_board_body(m["body"])
...
async def healthz() -> JSONResponse:
    ...
    # REST feeds that are covered by a push source -- skip staleness check
    # when push is healthy. 2026-09-07: derived from
    # ingest.failover.REST_FALLBACKS (the poller's own gate table) instead of
    # a hand-written copy that still carried "tfr": "push:stdds" and
    # "nas": "push:tfms" -- the same bogus 2026-06-07 POC mapping removed
    # from the poller and /api/v1/feeds on 2026-07-23, so /healthz had kept
    # hiding real tfr/nas staleness behind unrelated push feeds for six
    # more weeks. The heartbeat window is per feed (cover_max_age) so the
    # amtrak twin, which stamps once per 300 s poll, is not read as dead
    # by a fixed 300 s check.
    push_covers = _failover.push_covers()
        ...
        if age is None or age > t:
            ...
            if push_name:
                ...
                if push and push["fetched_at"] and not push["error"]:
                    ...
                    if push_age <= _failover.cover_max_age(name):
...
async def get_demo_readiness() -> JSONResponse:
    ...
    2026-09-20 Postgres cutover: this used to open
    /var/lib/corporatetraveldc/demo.db directly via its own ad-hoc
    sqlite3 connection, independently re-implementing the exact same
    readiness computation demo.recorder.seed_status() already had (a
    real, previously-duplicated logic pair — see src/demo/db.py's module
    docstring). Now calls the single shared implementation there instead
    of re-deriving it. seed_target/retention_days (14/364) match this
    route's own historical defaults, which happen to equal demo.recorder's
    own — kept as explicit literals here rather than an import so this
    route has no dependency on demo.recorder's requests/zlib-polling
    module.
    """
    from demo import db as _demo_db
    try:
        return JSONResponse(_demo_db.seed_status(seed_target=14, retention_days=364))
    except Exception as exc:
        return JSONResponse(
...
async def vault_research_read(request: Request, path: str = Query(...)) -> JSONResponse:
    """Tier-0-shaped (no Authorization header required) but X-Board-Key
    gated read access to second-brain RESEARCH content only -- for agent
    tools that cannot send an Authorization header (e.g. Cowork's fetch
    tool; confirmed live 2026-08-16 that board-write auth hits the
    identical Cloudflare-strips-Authorization wall). Contrast with
    /api/v1/vault/file (Tier-1, any vault path, HTML) -- this is
    deliberately narrower in scope AND now requires the same X-Board-Key
    credential board writes already do.

    2026-08-24: this endpoint had genuinely NO auth at all before today --
    a live pentest (docs/investor-materials/v1.5/PENTEST_2026-08-24.md,
    Finding N-1) confirmed it was reachable from the open internet via the
    pre-existing `dispatch-vault-research` Cloudflare Access bypass
    Application (same shape as `dispatch-board-public-bypass`), serving
    real vault content to anyone. Fixed by reusing X-Board-Key -- the
    identical mechanism board_post already uses -- rather than inventing a
    new credential; Cowork already holds this key for board writes, so its
    existing logic only needs to start sending the same header here too.
...
async def vault_research_list(request: Request, path: str = Query(default=_VAULT_RESEARCH_ROOT)) -> JSONResponse:
    ...
    Same scope rules and X-Board-Key gate as the read endpoint (see that
    endpoint's docstring, 2026-08-24 / Finding N-1); path defaults to the
...
async def osint_create_scope(
    ...
    tier: Tier = Depends(require_admin("osint.scope.create")),
) -> JSONResponse:
...
async def osint_update_scope(
    scope_id: int,
    body: dict,
    tier: Tier = Depends(require_admin("osint.scope.update")),
) -> JSONResponse:
...
async def osint_delete_scope(
    scope_id: int,
    tier: Tier = Depends(require_admin("osint.scope.delete")),
) -> JSONResponse:
...
# 2026-08-24 FIXED, corrected same pass: was hardcoded to 38.8521/-77.0377,
# a generic KDCA-area placeholder that never matched the real receiver --
# one of four independently-drifted placeholder coordinates found across
# this repo. First fix used FEEDER_LAT/FEEDER_LON -- wrong, that's the
# operator's FORMER residence (see runner/main.py's DEFAULT_LAT comment
# for the full writeup). ULTRAFEEDER_LAT/LON is the operator-confirmed
# TRUE current location. Falls back to the old placeholder only if unset.
_ADSB_DEFAULT_LAT = float(os.getenv("ULTRAFEEDER_LAT", 38.8521))
...
async def admin_healthz(
    tier: Tier = Depends(require_admin("admin.healthz"))
) -> JSONResponse:
...
async def admin_feeds(
    tier: Tier = Depends(require_admin("admin.feeds.list"))
) -> JSONResponse:
    feeds = db.get_feed_states()
...
async def admin_tokens(
    ...
    tier: Tier = Depends(require_admin("admin.tokens.list")),
) -> JSONResponse:
...
async def admin_version(
    tier: Tier = Depends(require_admin("admin.version"))
) -> JSONResponse:
    return JSONResponse({
        "version": "1.0.0",
...
async def admin_triggers(
    ...
    tier: Tier = Depends(require_admin("admin.triggers.list")),
) -> JSONResponse:
...
async def refresh_feed(
    feed_name: str,
    tier: Tier = Depends(require_admin("admin.feed.refresh")),
) -> JSONResponse:
    """
...
async def force_recompute_cps(
    tier: Tier = Depends(require_admin("admin.cps.force_recompute"))
) -> JSONResponse:
    trigger_id = str(uuid.uuid4())
    trigger_dir = pathlib.Path(config.trigger_dir())
    trigger_dir.mkdir(parents=True, exist_ok=True)
...
async def force_opsplan_snapshot(
    ...
    tier: Tier = Depends(require_admin("admin.opsplan.force_snapshot")),
) -> JSONResponse:
...
async def force_osint_scrape(
    tier: Tier = Depends(require_admin("admin.osint.force_scrape")),
) -> JSONResponse:
...
async def push_alert(
    ...
    tier: Tier = Depends(require_admin("admin.alert.push")),
) -> JSONResponse:
...
@app.get("/admin/vip")
async def get_vip(tier: Tier = Depends(require_admin("admin.vip.list"))) -> JSONResponse:
...
async def add_vip(
    body: VIPAddRequest,
    tier: Tier = Depends(require_admin("admin.vip.add")),
) -> JSONResponse:
...
async def delete_vip(
    entry: str,
    tier: Tier = Depends(require_admin("admin.vip.remove")),
) -> JSONResponse:
...
async def set_bandwidth_priority_route(
    ...
    tier: Tier = Depends(require_admin("admin.bandwidth_priority.set")),
) -> JSONResponse:
...
async def clear_bandwidth_priority_route(
    tier: Tier = Depends(require_admin("admin.bandwidth_priority.clear")),
) -> JSONResponse:
...
async def get_approval_request_route(
    request_id: str,
    tier: Tier = Depends(require_admin("admin.approval_request.get")),
) -> JSONResponse:
...
async def list_approval_requests_route(
    ...
    tier: Tier = Depends(require_admin("admin.approval_request.list")),
) -> JSONResponse:
...
async def watchdog_status(
    tier: Tier = Depends(require_admin("admin.watchdog.status")),
) -> JSONResponse:
