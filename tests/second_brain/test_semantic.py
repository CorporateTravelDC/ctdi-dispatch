...
def synthetic_index():
    ...
    # 2026-09-19: dict-key row access (row["col"]) is now the production
    # code's row-access style throughout compile.py (matching Postgres's
    # dict_row rows) -- sqlite3.Row supports both key and positional
    # access, so this keeps this test double compatible without any
    # tuple/dict branching in the functions under test.
    ...
    conn.executescript("""
        ...
        CREATE VIRTUAL TABLE vault_notes_fulltext USING fts5(
    ...
    for path, tags, ingest, title, content in rows:
        ...
        conn.execute(
            "INSERT INTO vault_notes_fulltext(path, title, content, tags) "
...
def test_unmapped_tags_are_surfaced(model, synthetic_index):
    ...
    silently dropped."""
    conn, _ = synthetic_index
    conn.execute("INSERT INTO vault_notes_fulltext(path, title, content, tags) "
                 "VALUES (?,?,?,?)",
...
def test_derivation_is_deterministic(synthetic_index):
    ...
    # Re-run against the SAME vault content (vault_documents/vault_notes_fulltext
